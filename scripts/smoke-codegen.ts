...
console.log(
  "Enabled:",
  myChain.enabled.map((n) => n.typeId),
);
...
result.passes.forEach((p, i) => {
  console.log(
    `  Pass ${i}: mode=${p.mode ?? "normal"} readsPrev=${p.readsPrevPass} nodes=[${p.nodeIds.join(", ")}]`,
  );
});
...
result.uniforms.forEach((u) =>
  console.log(
    `  ${u.name}  ${u.type}${u.arrayLength ? `[${u.arrayLength}]` : ""}  (${u.layerName}.${u.originalName})`,
  ),
);
...
console.log(
  "Restored types:",
  restored.nodes.map((n) => n.typeId),
);
console.log(
  "Configs equal?",
  JSON.stringify(restored.nodes[0].config) === JSON.stringify(myChain.nodes[0].config),
);
