...
console.log('Layer 0 nodes:', s.layers[0].nodes.map((n) => n.typeId))

console.log('\n=== Round-trip ===')
const json = s.toJSON()
console.log('Serialized layers:', json.layers.length, 'effects:', json.layers[0].effects.length)
const restored = Scene.fromJSON(json)
console.log('Restored:', restored.layers[0].source.typeId, restored.layers[0].effects.map((e) => e.typeId))
