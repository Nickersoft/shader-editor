...
import { GeneratorNode, EffectNode, getEffectScope, type EffectScope } from "@/shaders/core/node";
...
function SortableLayerRow({
  ...
  const { selectNode, toggleLayer, removeLayer, selectedNodeId } = useComposer();
  ...
  return (
    ...
    >
      ...
      <button className="flex-1 min-w-0 text-left" onClick={() => selectNode(layer.source.id)}>
        <div className="flex items-center gap-2">
          ...
          {layer.effects.length > 0 && (
            <span className="text-[10px] text-muted-foreground/70">{layer.effects.length}fx</span>
          )}
        </div>
      </button>
...
function SortableEffectRow({
  ...
  const dragId = scope === "layer" ? `effect:${layerId}:${effectId}` : `scene-effect:${effectId}`;
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({
    id: dragId,
  });
  ...
  return (
    ...
    >
      ...
      <button className="flex-1 min-w-0 text-left" onClick={() => selectNode(effectId)}>
        <div className="flex items-center gap-2">
...
function SortableLayerWithEffects({
  ...
  const displayEffects = useMemo(() => [...layer.effects].reverse(), [layer.effects]);

  const handleDragEnd = (event: DragEndEvent) => {
    const { active, over } = event;
    if (!over || active.id === over.id) return;
    const oldIndex = displayEffects.findIndex((e) => `effect:${layer.id}:${e.id}` === active.id);
    const newIndex = displayEffects.findIndex((e) => `effect:${layer.id}:${e.id}` === over.id);
  ...
  return (
    <div className="space-y-1.5">
      <SortableLayerRow layer={layer} isExpanded={isExpanded} onToggleExpanded={onToggleExpanded} />
      {isExpanded && (
...
export function LayerStack() {
  ...
  const displayLayers = useMemo(() => [...scene.layers].reverse(), [scene.layers]);
  const displayPostEffects = useMemo(() => [...scene.postEffects].reverse(), [scene.postEffects]);
  ...
  const handleLayerDragEnd = (event: DragEndEvent) => {
    const { active, over } = event;
    if (!over || active.id === over.id) return;
    const oldIndex = displayLayers.findIndex((l) => `layer:${l.id}` === active.id);
    const newIndex = displayLayers.findIndex((l) => `layer:${l.id}` === over.id);
  ...
  const handleSceneEffectDragEnd = (event: DragEndEvent) => {
    const { active, over } = event;
    if (!over || active.id === over.id) return;
    const oldIndex = displayPostEffects.findIndex((e) => `scene-effect:${e.id}` === active.id);
    const newIndex = displayPostEffects.findIndex((e) => `scene-effect:${e.id}` === over.id);
  ...
  const grouped = useMemo(() => {
    ...
    return CATEGORY_ORDER.map((cat) => ({
      ...
      items: (byCat.get(cat) ?? []).sort((a, b) => a.meta.name.localeCompare(b.meta.name)),
    ...
    if (addTarget?.kind === "newLayer" && isGeneratorClass(cls)) {
      ...
      if (isEffectClass(cls)) {
        ...
        if (scope === "scene") {
          ...
          const selected = selectedNodeId ? scene.findNode(selectedNodeId)?.layer : null;
    ...
    <div className="flex flex-col h-full min-h-0">
      <div className="border-b border-border shrink-0">
        <div className="flex items-center justify-between px-3 pt-3 mb-2">
          ...
          <div className="flex items-center gap-1">
            ...
            <Button
              ...
              disabled={scene.layers.length === 0 && scene.postEffects.length === 0}
        ...
        <div className="px-3 pb-2 flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <Input
              ...
              placeholder={selectionLabel ? selectionLabel + "…" : "Search primitives…"}
        ...
        <div className="px-3 pb-3 max-h-72 overflow-y-auto space-y-2">
          {grouped.length === 0 ? (
            <div className="text-xs text-muted-foreground py-2">No primitives match.</div>
          ) : (
            grouped.map(({ cat, meta, items }) => (
              <div key={cat}>
                <div className="text-[10px] uppercase tracking-wider text-muted-foreground mb-1 mt-1">
                  {meta.name} <span className="text-muted-foreground/60">({items.length})</span>
                </div>
                <div className="flex flex-wrap gap-1">
                  {items.map((cls) => (
                    <Button
