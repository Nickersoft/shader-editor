...
export function createShaderPipeline(
  ...
  let sceneBackground: [number, number, number, number] =
    options.sceneBackground ?? [0, 0, 0, 1]
  ...
  function render(
    ...
    for (let i = 0; i < passes.length; i++) {
      ...
      if (pass.bindLayerTextures) {
        ...
        firstFreeUnit = Math.max(
          firstFreeUnit,
          LAYER_TEXTURE_BASE_UNIT + layerTextures.length,
        )
      }
