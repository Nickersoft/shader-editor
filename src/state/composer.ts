...
export const useComposerStore = create<ComposerStore>((set) => {
  ...
  const applyScene = (
    ...
    selectedNodeId:
      selectedNodeId === undefined ? state.selectedNodeId : selectedNodeId,
  })
  ...
  return {
    ...
    loadScene: (scene) =>
      set((state) =>
        applyScene(state, scene, fallbackSelection(scene)),
      ),
    ...
    reorderLayers: (orderedIds) =>
      set((state) => {
        ...
        for (const l of state.scene.layers)
          if (!orderedIds.includes(l.id)) reordered.push(l)
        const next = snapshotScene(state.scene)
    ...
    reorderSceneEffects: (orderedIds) =>
      set((state) => {
        ...
        const reordered = orderedIds
          .map((id) => byId.get(id))
          .filter((e): e is EffectNode => Boolean(e))
        for (const e of next.postEffects)
          if (!orderedIds.includes(e.id)) reordered.push(e)
