<script lang="ts">
	...
	import TransformWidget from './transform-widget.svelte';
	...
	let config = $derived((layer?.source.config as Record<string, unknown>) ?? {});
	...
	>
		{#each controls as c, idx (idx)}
			...
			{:else if c.kind === 'boundingBox'}
				...
				{@const halfMul = c.halfExtent ? 1 : 0.5}
				{@const base = height}
				{@const halfW = ext ? ext.w * base : ((config[c.w] as number) ?? 0.5) * halfMul * base}
				{@const halfH = ext ? ext.h * base : ((config[c.h] as number) ?? 0.5) * halfMul * base}
				<g>
					<rect
						x={cx - halfW}
						y={cy - halfH}
