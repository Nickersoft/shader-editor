<script lang="ts">
	...
	import CanvasOverlay from './canvas-overlay.svelte';
	...
	function isPaletteValue(v: unknown): v is PaletteValue {
		return (
			typeof v === 'object' && v !== null && Array.isArray((v as PaletteValue).values)
		);
	}
	...
	$effect(() => {
		...
		const onPointerDown = (e: PointerEvent) => {
			...
			for (let i = layers.length - 1; i >= 0; i--) {
				...
				const cls = layer.source.cls as { spatialControls?: SpatialControlsSpec };
				const config = layer.source.config as Record<string, unknown>;
				if (hitTestLayerBody(cls.spatialControls, config, px, py, rect.width, rect.height)) {
		...
		const onPointerMove = (e: PointerEvent) => {
			if (!drag) return;
			const rect = canvas!.getBoundingClientRect();
			const dx = (e.clientX - rect.left - drag.startPx) / Math.max(drag.rectWidth, 1);
			const dy = -(e.clientY - rect.top - drag.startPy) / Math.max(drag.rectHeight, 1);
