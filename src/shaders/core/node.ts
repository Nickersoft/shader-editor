...
> {
  ...
  constructor(init: NodeInit = {}) {
    ...
    this.config = (init.config ? cls.config.parse(init.config) : cls.config.parse({})) as C
    this.inputs = (init.inputs ? cls.inputs.parse(init.inputs) : cls.inputs.parse({})) as I
  ...
  /** Sanitized prefix for this instance's GLSL uniforms. */
  get prefix(): string {
    return sanitizeName(this.id)
  }
