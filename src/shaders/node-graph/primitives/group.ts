...
class Group extends BasePrimitive<Config> {
  ...
  inputs(cfg: Record<string, unknown>): readonly PinSpec[] {
    ...
    return requirePrimitive(GROUP_INPUT_TYPE_ID).outputs(gi.config);
  }
  ...
  outputs(cfg: Record<string, unknown>): readonly PinSpec[] {
    ...
    return requirePrimitive(GROUP_OUTPUT_TYPE_ID).inputs(go.config);
  }
