...
import {
  S as set_nested_value,
  T as MUTATIVE_METHODS,
  _ as create_field_proxy,
  b as flatten_issues,
  o as handle_error_and_jsonify,
  v as deep_set,
  x as normalize_issue,
...
function query(validate_or_fn, maybe_fn) {
  ...
  const wrapper = (arg) => {
    ...
    const { event, state } = get_request_store();
    return create_query_resource(__, stringify_remote_arg(arg, state.transport), state, () =>
      run_remote_function(event, state, false, () => validate(arg), fn),
    );
  };
  Object.defineProperty(wrapper, "__", { value: __ });
...
function live(validate_or_fn, maybe_fn) {
  ...
  const wrapper = (arg) => {
    ...
    const { event, state } = get_request_store();
    return create_live_query_resource(__, stringify_remote_arg(arg, state.transport), state, () =>
  ...
  return {
    ...
    refresh() {
      ...
      return update_refresh_value(
        refresh_context,
        is_immediate_refresh ? get_promise() : fn(),
