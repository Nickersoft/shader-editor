var e = Object.defineProperty,
  t = (t, n) => {
    let r = {};
    for (var i in t) e(r, i, { get: t[i], enumerable: !0 });
    return (n || e(r, Symbol.toStringTag, { value: `Module` }), r);
  },
  n = Array.isArray,
  r = Array.prototype.indexOf,
  i = Array.prototype.includes,
  a = Array.from,
  o = Object.defineProperty,
  s = Object.getOwnPropertyDescriptor,
  c = Object.getOwnPropertyDescriptors,
  l = Object.prototype,
  u = Array.prototype,
  d = Object.getPrototypeOf,
  f = Object.isExtensible;
function p(e) {
  return typeof e == `function`;
}
var m = () => {};
function h(e) {
  for (var t = 0; t < e.length; t++) e[t]();
}
function g() {
  var e, t;
  return {
    promise: new Promise((n, r) => {
      ((e = n), (t = r));
    }),
    resolve: e,
    reject: t,
  };
}
function _(e, t) {
  if (Array.isArray(e)) return e;
  if (t === void 0 || !(Symbol.iterator in e)) return Array.from(e);
  let n = [];
  for (let r of e) if ((n.push(r), n.length === t)) break;
  return n;
}
var v = 1 << 24,
  y = 1024,
  b = 2048,
  x = 4096,
  S = 8192,
  ee = 16384,
  te = 32768,
  ne = 1 << 25,
  re = 65536,
  ie = 1 << 19,
  ae = 1 << 20,
  oe = 1 << 25,
  se = 65536,
  ce = 1 << 21,
  le = 1 << 22,
  ue = 1 << 23,
  de = Symbol(`$state`),
  fe = Symbol(`legacy props`),
  pe = Symbol(``),
  me = Symbol(`proxy path`),
  he = new (class extends Error {
    name = `StaleReactionError`;
    message = "The reaction that called `getAbortSignal()` was re-run or destroyed";
  })(),
  ge = !!globalThis.document?.contentType && globalThis.document.contentType.includes(`xml`);
function _e(e) {
  throw Error(`https://svelte.dev/e/experimental_async_required`);
}
function ve(e) {
  throw Error(`https://svelte.dev/e/lifecycle_outside_component`);
}
function ye() {
  throw Error(`https://svelte.dev/e/missing_context`);
}
function be() {
  throw Error(`https://svelte.dev/e/async_derived_orphan`);
}
function xe(e, t, n) {
  throw Error(`https://svelte.dev/e/each_key_duplicate`);
}
function Se(e) {
  throw Error(`https://svelte.dev/e/effect_in_teardown`);
}
function Ce() {
  throw Error(`https://svelte.dev/e/effect_in_unowned_derived`);
}
function we(e) {
  throw Error(`https://svelte.dev/e/effect_orphan`);
}
function Te() {
  throw Error(`https://svelte.dev/e/effect_update_depth_exceeded`);
}
function Ee() {
  throw Error(`https://svelte.dev/e/fork_discarded`);
}
function De() {
  throw Error(`https://svelte.dev/e/fork_timing`);
}
function Oe() {
  throw Error(`https://svelte.dev/e/get_abort_signal_outside_reaction`);
}
function ke() {
  throw Error(`https://svelte.dev/e/hydration_failed`);
}
function Ae(e) {
  throw Error(`https://svelte.dev/e/lifecycle_legacy_only`);
}
function je(e) {
  throw Error(`https://svelte.dev/e/props_invalid_value`);
}
function Me() {
  throw Error(`https://svelte.dev/e/set_context_after_init`);
}
function Ne() {
  throw Error(`https://svelte.dev/e/state_descriptors_fixed`);
}
function Pe() {
  throw Error(`https://svelte.dev/e/state_prototype_fixed`);
}
function Fe() {
  throw Error(`https://svelte.dev/e/state_unsafe_mutation`);
}
function Ie() {
  throw Error(`https://svelte.dev/e/svelte_boundary_reset_onerror`);
}
var Le = {},
  C = Symbol(),
  Re = `http://www.w3.org/1999/xhtml`,
  ze = `http://www.w3.org/2000/svg`,
  Be = `http://www.w3.org/1998/Math/MathML`,
  Ve = `@attach`;
function He() {
  console.warn(`https://svelte.dev/e/derived_inert`);
}
function Ue(e) {
  console.warn(`https://svelte.dev/e/hydratable_missing_but_expected`);
}
function We(e) {
  console.warn(`https://svelte.dev/e/hydration_mismatch`);
}
function Ge() {
  console.warn(`https://svelte.dev/e/select_multiple_invalid_value`);
}
function Ke() {
  console.warn(`https://svelte.dev/e/svelte_boundary_reset_noop`);
}
var w = !1;
function T(e) {
  w = e;
}
var E;
function D(e) {
  if (e === null) throw (We(), Le);
  return (E = e);
}
function O() {
  return D(z(E));
}
function qe(e) {
  if (w) {
    if (z(E) !== null) throw (We(), Le);
    E = e;
  }
}
function Je(e = 1) {
  if (w) {
    for (var t = e, n = E; t--; ) n = z(n);
    E = n;
  }
}
function Ye(e = !0) {
  for (var t = 0, n = E; ; ) {
    if (n.nodeType === 8) {
      var r = n.data;
      if (r === `]`) {
        if (t === 0) return n;
        --t;
      } else (r === `[` || r === `[!` || (r[0] === `[` && !isNaN(Number(r.slice(1))))) && (t += 1);
    }
    var i = z(n);
    (e && n.remove(), (n = i));
  }
}
function Xe(e) {
  if (!e || e.nodeType !== 8) throw (We(), Le);
  return e.data;
}
function Ze(e) {
  return e === this.v;
}
function Qe(e, t) {
  return e == e ? e !== t || (typeof e == `object` && !!e) || typeof e == `function` : t == t;
}
function $e(e) {
  return !Qe(e, this.v);
}
var k = !1,
  et = !1;
function tt(e, t) {
  return ((e.label = t), nt(e.v, t), e);
}
function nt(e, t) {
  return (e?.[me]?.(t), e);
}
function rt(e) {
  return typeof e == `symbol`
    ? `Symbol(${e.description})`
    : typeof e == `function`
      ? `<function>`
      : typeof e == `object` && e
        ? `<object>`
        : String(e);
}
var A = null;
function it(e) {
  A = e;
}
function at() {
  let e = {};
  return [() => (ct(e) || ye(), ot(e)), (t) => st(e, t)];
}
function ot(e) {
  return pt(`getContext`).get(e);
}
function st(e, t) {
  let n = pt(`setContext`);
  if (k) {
    var r = K.f;
    (!U && r & 32 && !A.i) || Me();
  }
  return (n.set(e, t), t);
}
function ct(e) {
  return pt(`hasContext`).has(e);
}
function lt() {
  return pt(`getAllContexts`);
}
function ut(e, t = !1, n) {
  A = {
    p: A,
    i: !1,
    c: null,
    e: null,
    s: e,
    x: null,
    r: K,
    l: et && !t ? { s: null, u: null, $: [] } : null,
  };
}
function dt(e) {
  var t = A,
    n = t.e;
  if (n !== null) {
    t.e = null;
    for (var r of n) Xn(r);
  }
  return (e !== void 0 && (t.x = e), (t.i = !0), (A = t.p), e ?? {});
}
function ft() {
  return !et || (A !== null && A.l === null);
}
function pt(e) {
  return (A === null && ve(e), (A.c ??= new Map(mt(A) || void 0)));
}
function mt(e) {
  let t = e.p;
  for (; t !== null; ) {
    let e = t.c;
    if (e !== null) return e;
    t = t.p;
  }
  return null;
}
var ht = [];
function gt() {
  var e = ht;
  ((ht = []), h(e));
}
function _t(e) {
  if (ht.length === 0 && !Mt) {
    var t = ht;
    queueMicrotask(() => {
      t === ht && gt();
    });
  }
  ht.push(e);
}
function vt() {
  for (; ht.length > 0; ) gt();
}
function yt(e) {
  var t = K;
  if (t === null) return ((U.f |= ue), e);
  if (!(t.f & 32768) && !(t.f & 4)) throw e;
  bt(e, t);
}
function bt(e, t) {
  for (; t !== null; ) {
    if (t.f & 128) {
      if (!(t.f & 32768)) throw e;
      try {
        t.b.error(e);
        return;
      } catch (t) {
        e = t;
      }
    }
    t = t.parent;
  }
  throw e;
}
var xt = ~(b | x | y);
function j(e, t) {
  e.f = (e.f & xt) | t;
}
function St(e) {
  e.f & 512 || e.deps === null ? j(e, y) : j(e, x);
}
function Ct(e) {
  if (e !== null) for (let t of e) !(t.f & 2) || !(t.f & 65536) || ((t.f ^= se), Ct(t.deps));
}
function wt(e, t, n) {
  (e.f & 2048 ? t.add(e) : e.f & 4096 && n.add(e), Ct(e.deps), j(e, y));
}
var Tt = [];
function Et(e, t = m) {
  let n = null,
    r = new Set();
  function i(t) {
    if (Qe(e, t) && ((e = t), n)) {
      let t = !Tt.length;
      for (let t of r) (t[1](), Tt.push(t, e));
      if (t) {
        for (let e = 0; e < Tt.length; e += 2) Tt[e][0](Tt[e + 1]);
        Tt.length = 0;
      }
    }
  }
  function a(t) {
    i(t(e));
  }
  function o(o, s = m) {
    let c = [o, s];
    return (
      r.add(c),
      r.size === 1 && (n = t(i, a) || m),
      o(e),
      () => {
        (r.delete(c), r.size === 0 && n && (n(), (n = null)));
      }
    );
  }
  return { set: i, update: a, subscribe: o };
}
var Dt = !1,
  Ot = !1;
function kt(e) {
  var t = Ot;
  try {
    return ((Ot = !1), [e(), Ot]);
  } finally {
    Ot = t;
  }
}
var M = new Set(),
  N = null,
  At = null,
  P = null,
  jt = null,
  Mt = !1,
  Nt = !1,
  Pt = null,
  Ft = null,
  It = 0,
  Lt = 1,
  Rt = class e {
    id = Lt++;
    current = new Map();
    previous = new Map();
    #e = new Set();
    #t = new Set();
    #n = new Set();
    #r = new Map();
    #i = new Map();
    #a = null;
    #o = [];
    #s = [];
    #c = new Set();
    #l = new Set();
    #u = new Map();
    #d = new Set();
    is_fork = !1;
    #f = !1;
    #p = new Set();
    #m() {
      return this.is_fork || this.#i.size > 0;
    }
    #h() {
      for (let n of this.#p)
        for (let r of n.#i.keys()) {
          for (var e = !1, t = r; t.parent !== null; ) {
            if (this.#u.has(t)) {
              e = !0;
              break;
            }
            t = t.parent;
          }
          if (!e) return !0;
        }
      return !1;
    }
    skip_effect(e) {
      (this.#u.has(e) || this.#u.set(e, { d: [], m: [] }), this.#d.delete(e));
    }
    unskip_effect(e, t = (e) => this.schedule(e)) {
      var n = this.#u.get(e);
      if (n) {
        this.#u.delete(e);
        for (var r of n.d) (j(r, b), t(r));
        for (r of n.m) (j(r, x), t(r));
      }
      this.#d.add(e);
    }
    #g() {
      if ((It++ > 1e3 && (M.delete(this), Bt()), !this.#m())) {
        for (let e of this.#c) (this.#l.delete(e), j(e, b), this.schedule(e));
        for (let e of this.#l) (j(e, x), this.schedule(e));
      }
      let t = this.#o;
      ((this.#o = []), this.apply());
      var n = (Pt = []),
        r = [],
        i = (Ft = []);
      for (let e of t)
        try {
          this.#_(e, n, r);
        } catch (t) {
          throw (qt(e), t);
        }
      if (((N = null), i.length > 0)) {
        var a = e.ensure();
        for (let e of i) a.schedule(e);
      }
      if (((Pt = null), (Ft = null), this.#m() || this.#h())) {
        (this.#v(r), this.#v(n));
        for (let [e, t] of this.#u) Kt(e, t);
      } else {
        (this.#r.size === 0 && M.delete(this), this.#c.clear(), this.#l.clear());
        for (let e of this.#e) e(this);
        (this.#e.clear(), (At = this), Vt(r), Vt(n), (At = null), this.#a?.resolve());
      }
      var o = N;
      if (this.#o.length > 0) {
        let e = (o ??= this);
        e.#o.push(...this.#o.filter((t) => !e.#o.includes(t)));
      }
      (o !== null && (M.add(o), o.#g()), k && !M.has(this) && this.#y());
    }
    #_(e, t, n) {
      e.f ^= y;
      for (var r = e.first; r !== null; ) {
        var i = r.f,
          a = (i & 96) != 0;
        if (!((a && i & 1024) || i & 8192 || this.#u.has(r)) && r.fn !== null) {
          a
            ? (r.f ^= y)
            : i & 4
              ? t.push(r)
              : k && i & 16777224
                ? n.push(r)
                : Dr(r) && (i & 16 && this.#l.add(r), Mr(r));
          var o = r.first;
          if (o !== null) {
            r = o;
            continue;
          }
        }
        for (; r !== null; ) {
          var s = r.next;
          if (s !== null) {
            r = s;
            break;
          }
          r = r.parent;
        }
      }
    }
    #v(e) {
      for (var t = 0; t < e.length; t += 1) wt(e[t], this.#c, this.#l);
    }
    capture(e, t, n = !1) {
      (e.v !== C && !this.previous.has(e) && this.previous.set(e, e.v),
        e.f & 8388608 || (this.current.set(e, [t, n]), P?.set(e, t)),
        this.is_fork || (e.v = t));
    }
    activate() {
      N = this;
    }
    deactivate() {
      ((N = null), (P = null));
    }
    flush() {
      try {
        ((Nt = !0), (N = this), this.#g());
      } finally {
        ((It = 0),
          (jt = null),
          (Pt = null),
          (Ft = null),
          (Nt = !1),
          (N = null),
          (P = null),
          mn.clear());
      }
    }
    discard() {
      for (let e of this.#t) e(this);
      (this.#t.clear(), this.#n.clear(), M.delete(this));
    }
    register_created_effect(e) {
      this.#s.push(e);
    }
    #y() {
      for (let l of M) {
        var e = l.id < this.id,
          t = [];
        for (let [r, [i, a]] of this.current) {
          if (l.current.has(r)) {
            var n = l.current.get(r)[0];
            if (e && i !== n) l.current.set(r, [i, a]);
            else continue;
          }
          t.push(r);
        }
        var r = [...l.current.keys()].filter((e) => !this.current.has(e));
        if (r.length === 0) e && l.discard();
        else if (t.length > 0) {
          if (e)
            for (let e of this.#d)
              l.unskip_effect(e, (e) => {
                e.f & 4194320 ? l.schedule(e) : l.#v([e]);
              });
          l.activate();
          var i = new Set(),
            a = new Map();
          for (var o of t) Ht(o, r, i, a);
          a = new Map();
          var s = [...l.current.keys()].filter((e) =>
            this.current.has(e) ? this.current.get(e)[0] !== e : !0,
          );
          for (let e of this.#s)
            !(e.f & 155648) &&
              Wt(e, s, a) &&
              (e.f & 4194320 ? (j(e, b), l.schedule(e)) : l.#c.add(e));
          if (l.#o.length > 0) {
            l.apply();
            for (var c of l.#o) l.#_(c, [], []);
            l.#o = [];
          }
          l.deactivate();
        }
      }
      for (let e of M)
        e.#p.has(this) && (e.#p.delete(this), e.#p.size === 0 && !e.#m() && (e.activate(), e.#g()));
    }
    increment(e, t) {
      let n = this.#r.get(t) ?? 0;
      if ((this.#r.set(t, n + 1), e)) {
        let e = this.#i.get(t) ?? 0;
        this.#i.set(t, e + 1);
      }
    }
    decrement(e, t, n) {
      let r = this.#r.get(t) ?? 0;
      if ((r === 1 ? this.#r.delete(t) : this.#r.set(t, r - 1), e)) {
        let e = this.#i.get(t) ?? 0;
        e === 1 ? this.#i.delete(t) : this.#i.set(t, e - 1);
      }
      this.#f ||
        n ||
        ((this.#f = !0),
        _t(() => {
          ((this.#f = !1), this.flush());
        }));
    }
    transfer_effects(e, t) {
      for (let t of e) this.#c.add(t);
      for (let e of t) this.#l.add(e);
      (e.clear(), t.clear());
    }
    oncommit(e) {
      this.#e.add(e);
    }
    ondiscard(e) {
      this.#t.add(e);
    }
    on_fork_commit(e) {
      this.#n.add(e);
    }
    run_fork_commit_callbacks() {
      for (let e of this.#n) e(this);
      this.#n.clear();
    }
    settled() {
      return (this.#a ??= g()).promise;
    }
    static ensure() {
      if (N === null) {
        let t = (N = new e());
        Nt ||
          (M.add(N),
          Mt ||
            _t(() => {
              N === t && t.flush();
            }));
      }
      return N;
    }
    apply() {
      if (!k || (!this.is_fork && M.size === 1)) {
        P = null;
        return;
      }
      P = new Map();
      for (let [e, [t]] of this.current) P.set(e, t);
      for (let n of M)
        if (!(n === this || n.is_fork)) {
          var e = !1,
            t = !1;
          if (n.id < this.id)
            for (let [r, [, i]] of n.current)
              i || ((e ||= this.current.has(r)), (t ||= !this.current.has(r)));
          if (e && t) this.#p.add(n);
          else for (let [e, t] of n.previous) P.has(e) || P.set(e, t);
        }
    }
    schedule(e) {
      if (((jt = e), e.b?.is_pending && e.f & 16777228 && !(e.f & 32768))) {
        e.b.defer_effect(e);
        return;
      }
      for (var t = e; t.parent !== null; ) {
        t = t.parent;
        var n = t.f;
        if (Pt !== null && t === K && (k || ((U === null || !(U.f & 2)) && !Dt))) return;
        if (n & 96) {
          if (!(n & 1024)) return;
          t.f ^= y;
        }
      }
      this.#o.push(t);
    }
  };
function zt(e) {
  var t = Mt;
  Mt = !0;
  try {
    var n;
    for (e && (N !== null && !N.is_fork && N.flush(), (n = e())); ; ) {
      if ((vt(), N === null)) return n;
      N.flush();
    }
  } finally {
    Mt = t;
  }
}
function Bt() {
  try {
    Te();
  } catch (e) {
    bt(e, jt);
  }
}
var F = null;
function Vt(e) {
  var t = e.length;
  if (t !== 0) {
    for (var n = 0; n < t; ) {
      var r = e[n++];
      if (
        !(r.f & 24576) &&
        Dr(r) &&
        ((F = new Set()),
        Mr(r),
        r.deps === null &&
          r.first === null &&
          r.nodes === null &&
          r.teardown === null &&
          r.ac === null &&
          ur(r),
        F?.size > 0)
      ) {
        mn.clear();
        for (let e of F) {
          if (e.f & 24576) continue;
          let t = [e],
            n = e.parent;
          for (; n !== null; ) (F.has(n) && (F.delete(n), t.push(n)), (n = n.parent));
          for (let e = t.length - 1; e >= 0; e--) {
            let n = t[e];
            n.f & 24576 || Mr(n);
          }
        }
        F.clear();
      }
    }
    F = null;
  }
}
function Ht(e, t, n, r) {
  if (!n.has(e) && (n.add(e), e.reactions !== null))
    for (let i of e.reactions) {
      let e = i.f;
      e & 2 ? Ht(i, t, n, r) : e & 4194320 && !(e & 2048) && Wt(i, t, r) && (j(i, b), Gt(i));
    }
}
function Ut(e, t) {
  if (e.reactions !== null)
    for (let n of e.reactions) {
      let e = n.f;
      e & 2 ? Ut(n, t) : e & 131072 && (j(n, b), t.add(n));
    }
}
function Wt(e, t, n) {
  let r = n.get(e);
  if (r !== void 0) return r;
  if (e.deps !== null)
    for (let r of e.deps) {
      if (i.call(t, r)) return !0;
      if (r.f & 2 && Wt(r, t, n)) return (n.set(r, !0), !0);
    }
  return (n.set(e, !1), !1);
}
function Gt(e) {
  N.schedule(e);
}
function Kt(e, t) {
  if (!(e.f & 32 && e.f & 1024)) {
    (e.f & 2048 ? t.d.push(e) : e.f & 4096 && t.m.push(e), j(e, y));
    for (var n = e.first; n !== null; ) (Kt(n, t), (n = n.next));
  }
}
function qt(e) {
  j(e, y);
  for (var t = e.first; t !== null; ) (qt(t), (t = t.next));
}
function Jt(e) {
  (k || _e(`fork`), N !== null && De());
  var t = Rt.ensure();
  ((t.is_fork = !0), (P = new Map()));
  var n = !1,
    r = t.settled();
  return (
    zt(e),
    {
      commit: async () => {
        if (n) {
          await r;
          return;
        }
        (M.has(t) || Ee(), (n = !0), (t.is_fork = !1));
        for (var [e, [i]] of t.current) ((e.v = i), (e.wv = Er()));
        (t.activate(),
          t.run_fork_commit_callbacks(),
          t.deactivate(),
          zt(() => {
            var e = new Set();
            for (var n of t.current.keys()) Ut(n, e);
            (hn(e), xn());
          }),
          t.flush(),
          await r);
      },
      discard: () => {
        for (var e of t.current.keys()) e.wv = Er();
        !n && M.has(t) && t.discard();
      },
    }
  );
}
function Yt(e) {
  let t = 0,
    n = _n(0),
    r;
  return () => {
    qn() &&
      (Q(n),
      nr(
        () => (
          t === 0 && (r = Lr(() => e(() => Cn(n)))),
          (t += 1),
          () => {
            _t(() => {
              (--t, t === 0 && (r?.(), (r = void 0), Cn(n)));
            });
          }
        ),
      ));
  };
}
var Xt = re | ie;
function Zt(e, t, n, r) {
  new Qt(e, t, n, r);
}
var Qt = class {
  parent;
  is_pending = !1;
  transform_error;
  #e;
  #t = w ? E : null;
  #n;
  #r;
  #i;
  #a = null;
  #o = null;
  #s = null;
  #c = null;
  #l = 0;
  #u = 0;
  #d = !1;
  #f = new Set();
  #p = new Set();
  #m = null;
  #h = Yt(
    () => (
      (this.#m = _n(this.#l)),
      () => {
        this.#m = null;
      }
    ),
  );
  constructor(e, t, n, r) {
    ((this.#e = e),
      (this.#n = t),
      (this.#r = (e) => {
        var t = K;
        ((t.b = this), (t.f |= 128), n(e));
      }),
      (this.parent = K.b),
      (this.transform_error = r ?? this.parent?.transform_error ?? ((e) => e)),
      (this.#i = ir(() => {
        if (w) {
          let e = this.#t;
          O();
          let t = e.data === `[!`;
          if (e.data.startsWith(`[?`)) {
            let t = JSON.parse(e.data.slice(2));
            this.#_(t);
          } else t ? this.#v() : this.#g();
        } else this.#y();
      }, Xt)),
      w && (this.#e = E));
  }
  #g() {
    try {
      this.#a = V(() => this.#r(this.#e));
    } catch (e) {
      this.error(e);
    }
  }
  #_(e) {
    let t = this.#n.failed;
    t &&
      (this.#s = V(() => {
        t(
          this.#e,
          () => e,
          () => () => {},
        );
      }));
  }
  #v() {
    let e = this.#n.pending;
    e &&
      ((this.is_pending = !0),
      (this.#o = V(() => e(this.#e))),
      _t(() => {
        var e = (this.#c = document.createDocumentFragment()),
          t = L();
        (e.append(t),
          (this.#a = this.#x(() => V(() => this.#r(t)))),
          this.#u === 0 &&
            (this.#e.before(e),
            (this.#c = null),
            dr(this.#o, () => {
              this.#o = null;
            }),
            this.#b(N)));
      }));
  }
  #y() {
    try {
      if (
        ((this.is_pending = this.has_pending_snippet()),
        (this.#u = 0),
        (this.#l = 0),
        (this.#a = V(() => {
          this.#r(this.#e);
        })),
        this.#u > 0)
      ) {
        var e = (this.#c = document.createDocumentFragment());
        hr(this.#a, e);
        let t = this.#n.pending;
        this.#o = V(() => t(this.#e));
      } else this.#b(N);
    } catch (e) {
      this.error(e);
    }
  }
  #b(e) {
    ((this.is_pending = !1), e.transfer_effects(this.#f, this.#p));
  }
  defer_effect(e) {
    wt(e, this.#f, this.#p);
  }
  is_rendered() {
    return !this.is_pending && (!this.parent || this.parent.is_rendered());
  }
  has_pending_snippet() {
    return !!this.#n.pending;
  }
  #x(e) {
    var t = K,
      n = U,
      r = A;
    (q(this.#i), G(this.#i), it(this.#i.ctx));
    try {
      return (Rt.ensure(), e());
    } catch (e) {
      return (yt(e), null);
    } finally {
      (q(t), G(n), it(r));
    }
  }
  #S(e, t) {
    if (!this.has_pending_snippet()) {
      this.parent && this.parent.#S(e, t);
      return;
    }
    ((this.#u += e),
      this.#u === 0 &&
        (this.#b(t),
        this.#o &&
          dr(this.#o, () => {
            this.#o = null;
          }),
        (this.#c &&= (this.#e.before(this.#c), null))));
  }
  update_pending_count(e, t) {
    (this.#S(e, t),
      (this.#l += e),
      !(!this.#m || this.#d) &&
        ((this.#d = !0),
        _t(() => {
          ((this.#d = !1), this.#m && bn(this.#m, this.#l));
        })));
  }
  get_effect_pending() {
    return (this.#h(), Q(this.#m));
  }
  error(e) {
    if (!this.#n.onerror && !this.#n.failed) throw e;
    N?.is_fork
      ? (this.#a && N.skip_effect(this.#a),
        this.#o && N.skip_effect(this.#o),
        this.#s && N.skip_effect(this.#s),
        N.on_fork_commit(() => {
          this.#C(e);
        }))
      : this.#C(e);
  }
  #C(e) {
    ((this.#a &&= (H(this.#a), null)),
      (this.#o &&= (H(this.#o), null)),
      (this.#s &&= (H(this.#s), null)),
      w && (D(this.#t), Je(), D(Ye())));
    var t = this.#n.onerror;
    let n = this.#n.failed;
    var r = !1,
      i = !1;
    let a = () => {
        if (r) {
          Ke();
          return;
        }
        ((r = !0),
          i && Ie(),
          this.#s !== null &&
            dr(this.#s, () => {
              this.#s = null;
            }),
          this.#x(() => {
            this.#y();
          }));
      },
      o = (e) => {
        try {
          ((i = !0), t?.(e, a), (i = !1));
        } catch (e) {
          bt(e, this.#i && this.#i.parent);
        }
        n &&
          (this.#s = this.#x(() => {
            try {
              return V(() => {
                var t = K;
                ((t.b = this),
                  (t.f |= 128),
                  n(
                    this.#e,
                    () => e,
                    () => a,
                  ));
              });
            } catch (e) {
              return (bt(e, this.#i.parent), null);
            }
          }));
      };
    _t(() => {
      var t;
      try {
        t = this.transform_error(e);
      } catch (e) {
        bt(e, this.#i && this.#i.parent);
        return;
      }
      typeof t == `object` && t && typeof t.then == `function`
        ? t.then(o, (e) => bt(e, this.#i && this.#i.parent))
        : o(t);
    });
  }
};
function $t(e, t, n, r) {
  let i = ft() ? rn : sn;
  var a = e.filter((e) => !e.settled);
  if (n.length === 0 && a.length === 0) {
    r(t.map(i));
    return;
  }
  var o = K,
    s = en(),
    c = a.length === 1 ? a[0].promise : a.length > 1 ? Promise.all(a.map((e) => e.promise)) : null;
  function l(e) {
    s();
    try {
      r(e);
    } catch (e) {
      o.f & 16384 || bt(e, o);
    }
    tn();
  }
  if (n.length === 0) {
    c.then(() => l(t.map(i)));
    return;
  }
  var u = nn();
  function d() {
    Promise.all(n.map((e) => an(e)))
      .then((e) => l([...t.map(i), ...e]))
      .catch((e) => bt(e, o))
      .finally(() => u());
  }
  c
    ? c.then(() => {
        (s(), d(), tn());
      })
    : d();
}
function en() {
  var e = K,
    t = U,
    n = A,
    r = N;
  return function (i = !0) {
    (q(e), G(t), it(n), i && !(e.f & 16384) && (r?.activate(), r?.apply()));
  };
}
function tn(e = !0) {
  (q(null), G(null), it(null), e && N?.deactivate());
}
function nn() {
  var e = K,
    t = e.b,
    n = N,
    r = t.is_rendered();
  return (
    t.update_pending_count(1, n),
    n.increment(r, e),
    (i = !1) => {
      (t.update_pending_count(-1, n), n.decrement(r, e, i));
    }
  );
}
function rn(e) {
  var t = 2 | b;
  return (
    K !== null && (K.f |= ie),
    {
      ctx: A,
      deps: null,
      effects: null,
      equals: Ze,
      f: t,
      fn: e,
      reactions: null,
      rv: 0,
      v: C,
      wv: 0,
      parent: K,
      ac: null,
    }
  );
}
function an(e, t, n) {
  let r = K;
  r === null && be();
  var i = void 0,
    a = _n(C),
    o = !U,
    s = new Map();
  return (
    tr(() => {
      var t = K,
        n = g();
      i = n.promise;
      try {
        Promise.resolve(e()).then(n.resolve, n.reject).finally(tn);
      } catch (e) {
        (n.reject(e), tn());
      }
      var c = N;
      if (o) {
        if (t.f & 32768) var l = nn();
        if (r.b.is_rendered()) (s.get(c)?.reject(he), s.delete(c));
        else {
          for (let e of s.values()) e.reject(he);
          s.clear();
        }
        s.set(c, n);
      }
      let u = (e, n = void 0) => {
        if ((l && l(n === he), !(n === he || t.f & 16384))) {
          if ((c.activate(), n)) ((a.f |= ue), bn(a, n));
          else {
            (a.f & 8388608 && (a.f ^= ue), bn(a, e));
            for (let [e, t] of s) {
              if ((s.delete(e), e === c)) break;
              t.reject(he);
            }
          }
          c.deactivate();
        }
      };
      n.promise.then(u, (e) => u(null, e || `unknown`));
    }),
    Jn(() => {
      for (let e of s.values()) e.reject(he);
    }),
    new Promise((e) => {
      function t(n) {
        function r() {
          n === i ? e(a) : t(i);
        }
        n.then(r, r);
      }
      t(i);
    })
  );
}
function on(e) {
  let t = rn(e);
  return (k || br(t), t);
}
function sn(e) {
  let t = rn(e);
  return ((t.equals = $e), t);
}
function cn(e) {
  var t = e.effects;
  if (t !== null) {
    e.effects = null;
    for (var n = 0; n < t.length; n += 1) H(t[n]);
  }
}
function ln(e) {
  var t,
    n = K,
    r = e.parent;
  if (!vr && r !== null && r.f & 24576) return (He(), e.v);
  q(r);
  try {
    ((e.f &= ~se), cn(e), (t = kr(e)));
  } finally {
    q(n);
  }
  return t;
}
function un(e) {
  var t = ln(e);
  if (
    !e.equals(t) &&
    ((e.wv = Er()),
    (!N?.is_fork || e.deps === null) &&
      (N === null ? (e.v = t) : N.capture(e, t, !0), e.deps === null))
  ) {
    j(e, y);
    return;
  }
  vr || (P === null ? St(e) : (qn() || N?.is_fork) && P.set(e, t));
}
function dn(e) {
  if (e.effects !== null)
    for (let t of e.effects)
      (t.teardown || t.ac) &&
        (t.teardown?.(), t.ac?.abort(he), (t.teardown = m), (t.ac = null), jr(t, 0), sr(t));
}
function fn(e) {
  if (e.effects !== null) for (let t of e.effects) t.teardown && Mr(t);
}
var pn = new Set(),
  mn = new Map();
function hn(e) {
  pn = e;
}
var gn = !1;
function _n(e, t) {
  return { f: 0, v: e, reactions: null, equals: Ze, rv: 0, wv: 0 };
}
function vn(e, t) {
  let n = _n(e, t);
  return (br(n), n);
}
function yn(e, t = !1, n = !0) {
  let r = _n(e);
  return (t || (r.equals = $e), et && n && A !== null && A.l !== null && (A.l.s ??= []).push(r), r);
}
function I(e, t, n = !1) {
  return (
    U !== null &&
      (!W || U.f & 131072) &&
      ft() &&
      U.f & 4325394 &&
      (J === null || !i.call(J, e)) &&
      Fe(),
    bn(e, n ? Tn(t) : t, Ft)
  );
}
function bn(e, t, n = null) {
  if (!e.equals(t)) {
    mn.set(e, vr ? t : e.v);
    var r = Rt.ensure();
    if ((r.capture(e, t), e.f & 2)) {
      let t = e;
      (e.f & 2048 && ln(t), P === null && St(t));
    }
    ((e.wv = Er()),
      wn(e, b, n),
      ft() && K !== null && K.f & 1024 && !(K.f & 96) && (Z === null ? xr([e]) : Z.push(e)),
      !r.is_fork && pn.size > 0 && !gn && xn());
  }
  return t;
}
function xn() {
  gn = !1;
  for (let e of pn) (e.f & 1024 && j(e, x), Dr(e) && Mr(e));
  pn.clear();
}
function Sn(e, t = 1) {
  var n = Q(e),
    r = t === 1 ? n++ : n--;
  return (I(e, n), r);
}
function Cn(e) {
  I(e, e.v + 1);
}
function wn(e, t, n) {
  var r = e.reactions;
  if (r !== null)
    for (var i = ft(), a = r.length, o = 0; o < a; o++) {
      var s = r[o],
        c = s.f;
      if (!(!i && s === K)) {
        var l = (c & b) === 0;
        if ((l && j(s, t), c & 2)) {
          var u = s;
          (P?.delete(u),
            c & 65536 || (c & 512 && (K === null || !(K.f & 2097152)) && (s.f |= se), wn(u, x, n)));
        } else if (l) {
          var d = s;
          (c & 16 && F !== null && F.add(d), n === null ? Gt(d) : n.push(d));
        }
      }
    }
}
function Tn(e) {
  if (typeof e != `object` || !e || de in e) return e;
  let t = d(e);
  if (t !== l && t !== u) return e;
  var r = new Map(),
    i = n(e),
    a = vn(0),
    o = null,
    c = wr,
    f = (e) => {
      if (wr === c) return e();
      var t = U,
        n = wr;
      (G(null), Tr(c));
      var r = e();
      return (G(t), Tr(n), r);
    };
  return (
    i && r.set(`length`, vn(e.length, o)),
    new Proxy(e, {
      defineProperty(e, t, n) {
        (!(`value` in n) || n.configurable === !1 || n.enumerable === !1 || n.writable === !1) &&
          Ne();
        var i = r.get(t);
        return (
          i === void 0
            ? f(() => {
                var e = vn(n.value, o);
                return (r.set(t, e), e);
              })
            : I(i, n.value, !0),
          !0
        );
      },
      deleteProperty(e, t) {
        var n = r.get(t);
        if (n === void 0) {
          if (t in e) {
            let e = f(() => vn(C, o));
            (r.set(t, e), Cn(a));
          }
        } else (I(n, C), Cn(a));
        return !0;
      },
      get(t, n, i) {
        if (n === de) return e;
        var a = r.get(n),
          c = n in t;
        if (
          (a === void 0 &&
            (!c || s(t, n)?.writable) &&
            ((a = f(() => vn(Tn(c ? t[n] : C), o))), r.set(n, a)),
          a !== void 0)
        ) {
          var l = Q(a);
          return l === C ? void 0 : l;
        }
        return Reflect.get(t, n, i);
      },
      getOwnPropertyDescriptor(e, t) {
        var n = Reflect.getOwnPropertyDescriptor(e, t);
        if (n && `value` in n) {
          var i = r.get(t);
          i && (n.value = Q(i));
        } else if (n === void 0) {
          var a = r.get(t),
            o = a?.v;
          if (a !== void 0 && o !== C)
            return { enumerable: !0, configurable: !0, value: o, writable: !0 };
        }
        return n;
      },
      has(e, t) {
        if (t === de) return !0;
        var n = r.get(t),
          i = (n !== void 0 && n.v !== C) || Reflect.has(e, t);
        return (n !== void 0 || (K !== null && (!i || s(e, t)?.writable))) &&
          (n === void 0 && ((n = f(() => vn(i ? Tn(e[t]) : C, o))), r.set(t, n)), Q(n) === C)
          ? !1
          : i;
      },
      set(e, t, n, c) {
        var l = r.get(t),
          u = t in e;
        if (i && t === `length`)
          for (var d = n; d < l.v; d += 1) {
            var p = r.get(d + ``);
            p === void 0 ? d in e && ((p = f(() => vn(C, o))), r.set(d + ``, p)) : I(p, C);
          }
        if (l === void 0)
          (!u || s(e, t)?.writable) && ((l = f(() => vn(void 0, o))), I(l, Tn(n)), r.set(t, l));
        else {
          u = l.v !== C;
          var m = f(() => Tn(n));
          I(l, m);
        }
        var h = Reflect.getOwnPropertyDescriptor(e, t);
        if ((h?.set && h.set.call(c, n), !u)) {
          if (i && typeof t == `string`) {
            var g = r.get(`length`),
              _ = Number(t);
            Number.isInteger(_) && _ >= g.v && I(g, _ + 1);
          }
          Cn(a);
        }
        return !0;
      },
      ownKeys(e) {
        Q(a);
        var t = Reflect.ownKeys(e).filter((e) => {
          var t = r.get(e);
          return t === void 0 || t.v !== C;
        });
        for (var [n, i] of r) i.v !== C && !(n in e) && t.push(n);
        return t;
      },
      setPrototypeOf() {
        Pe();
      },
    })
  );
}
function En(e) {
  try {
    if (typeof e == `object` && e && de in e) return e[de];
  } catch {}
  return e;
}
function Dn(e, t) {
  return Object.is(En(e), En(t));
}
var On, kn, An, jn;
function Mn() {
  if (On === void 0) {
    ((On = window), (kn = /Firefox/.test(navigator.userAgent)));
    var e = Element.prototype,
      t = Node.prototype,
      n = Text.prototype;
    ((An = s(t, `firstChild`).get),
      (jn = s(t, `nextSibling`).get),
      f(e) &&
        ((e.__click = void 0),
        (e.__className = void 0),
        (e.__attributes = null),
        (e.__style = void 0),
        (e.__e = void 0)),
      f(n) && (n.__t = void 0));
  }
}
function L(e = ``) {
  return document.createTextNode(e);
}
function R(e) {
  return An.call(e);
}
function z(e) {
  return jn.call(e);
}
function Nn(e, t) {
  if (!w) return R(e);
  var n = R(E);
  if (n === null) n = E.appendChild(L());
  else if (t && n.nodeType !== 3) {
    var r = L();
    return (n?.before(r), D(r), r);
  }
  return (t && zn(n), D(n), n);
}
function Pn(e, t = !1) {
  if (!w) {
    var n = R(e);
    return n instanceof Comment && n.data === `` ? z(n) : n;
  }
  if (t) {
    if (E?.nodeType !== 3) {
      var r = L();
      return (E?.before(r), D(r), r);
    }
    zn(E);
  }
  return E;
}
function Fn(e, t = 1, n = !1) {
  let r = w ? E : e;
  for (var i; t--; ) ((i = r), (r = z(r)));
  if (!w) return r;
  if (n) {
    if (r?.nodeType !== 3) {
      var a = L();
      return (r === null ? i?.after(a) : r.before(a), D(a), a);
    }
    zn(r);
  }
  return (D(r), r);
}
function In(e) {
  e.textContent = ``;
}
function Ln() {
  return !k || F !== null ? !1 : (K.f & te) !== 0;
}
function Rn(e, t, n) {
  let r = n ? { is: n } : void 0;
  return document.createElementNS(t ?? `http://www.w3.org/1999/xhtml`, e, r);
}
function zn(e) {
  if (e.nodeValue.length < 65536) return;
  let t = e.nextSibling;
  for (; t !== null && t.nodeType === 3; )
    (t.remove(), (e.nodeValue += t.nodeValue), (t = e.nextSibling));
}
function Bn(e, t) {
  if (t) {
    let t = document.body;
    ((e.autofocus = !0),
      _t(() => {
        document.activeElement === t && e.focus();
      }));
  }
}
var Vn = !1;
function Hn() {
  Vn ||
    ((Vn = !0),
    document.addEventListener(
      `reset`,
      (e) => {
        Promise.resolve().then(() => {
          if (!e.defaultPrevented) for (let t of e.target.elements) t.__on_r?.();
        });
      },
      { capture: !0 },
    ));
}
function Un(e) {
  var t = U,
    n = K;
  (G(null), q(null));
  try {
    return e();
  } finally {
    (G(t), q(n));
  }
}
function Wn(e, t, n, r = n) {
  e.addEventListener(t, () => Un(n));
  let i = e.__on_r;
  (i
    ? (e.__on_r = () => {
        (i(), r(!0));
      })
    : (e.__on_r = () => r(!0)),
    Hn());
}
function Gn(e) {
  (K === null && (U === null && we(e), Ce()), vr && Se(e));
}
function Kn(e, t) {
  var n = t.last;
  n === null ? (t.last = t.first = e) : ((n.next = e), (e.prev = n), (t.last = e));
}
function B(e, t) {
  var n = K;
  n !== null && n.f & 8192 && (e |= S);
  var r = {
    ctx: A,
    deps: null,
    nodes: null,
    f: e | b | 512,
    first: null,
    fn: t,
    last: null,
    next: null,
    parent: n,
    b: n && n.b,
    prev: null,
    teardown: null,
    wv: 0,
    ac: null,
  };
  N?.register_created_effect(r);
  var i = r;
  if (e & 4) Pt === null ? Rt.ensure().schedule(r) : Pt.push(r);
  else if (t !== null) {
    try {
      Mr(r);
    } catch (e) {
      throw (H(r), e);
    }
    i.deps === null &&
      i.teardown === null &&
      i.nodes === null &&
      i.first === i.last &&
      !(i.f & 524288) &&
      ((i = i.first), e & 16 && e & 65536 && i !== null && (i.f |= re));
  }
  if (i !== null && ((i.parent = n), n !== null && Kn(i, n), U !== null && U.f & 2 && !(e & 64))) {
    var a = U;
    (a.effects ??= []).push(i);
  }
  return r;
}
function qn() {
  return U !== null && !W;
}
function Jn(e) {
  let t = B(8, null);
  return (j(t, y), (t.teardown = e), t);
}
function Yn(e) {
  Gn(`$effect`);
  var t = K.f;
  if (!U && t & 32 && !(t & 32768)) {
    var n = A;
    (n.e ??= []).push(e);
  } else return Xn(e);
}
function Xn(e) {
  return B(4 | ae, e);
}
function Zn(e) {
  return (Gn(`$effect.pre`), B(8 | ae, e));
}
function Qn(e) {
  Rt.ensure();
  let t = B(64 | ie, e);
  return () => {
    H(t);
  };
}
function $n(e) {
  Rt.ensure();
  let t = B(64 | ie, e);
  return (e = {}) =>
    new Promise((n) => {
      e.outro
        ? dr(t, () => {
            (H(t), n(void 0));
          })
        : (H(t), n(void 0));
    });
}
function er(e) {
  return B(4, e);
}
function tr(e) {
  return B(le | ie, e);
}
function nr(e, t = 0) {
  return B(8 | t, e);
}
function rr(e, t = [], n = [], r = []) {
  $t(r, t, n, (t) => {
    B(8, () => e(...t.map(Q)));
  });
}
function ir(e, t = 0) {
  return B(16 | t, e);
}
function ar(e, t = 0) {
  return B(v | t, e);
}
function V(e) {
  return B(32 | ie, e);
}
function or(e) {
  var t = e.teardown;
  if (t !== null) {
    let e = vr,
      n = U;
    (yr(!0), G(null));
    try {
      t.call(null);
    } finally {
      (yr(e), G(n));
    }
  }
}
function sr(e, t = !1) {
  var n = e.first;
  for (e.first = e.last = null; n !== null; ) {
    let e = n.ac;
    e !== null &&
      Un(() => {
        e.abort(he);
      });
    var r = n.next;
    (n.f & 64 ? (n.parent = null) : H(n, t), (n = r));
  }
}
function cr(e) {
  for (var t = e.first; t !== null; ) {
    var n = t.next;
    (t.f & 32 || H(t), (t = n));
  }
}
function H(e, t = !0) {
  var n = !1;
  ((t || e.f & 262144) &&
    e.nodes !== null &&
    e.nodes.end !== null &&
    (lr(e.nodes.start, e.nodes.end), (n = !0)),
    j(e, ne),
    sr(e, t && !n),
    jr(e, 0));
  var r = e.nodes && e.nodes.t;
  if (r !== null) for (let e of r) e.stop();
  (or(e), (e.f ^= ne), (e.f |= ee));
  var i = e.parent;
  (i !== null && i.first !== null && ur(e),
    (e.next = e.prev = e.teardown = e.ctx = e.deps = e.fn = e.nodes = e.ac = e.b = null));
}
function lr(e, t) {
  for (; e !== null; ) {
    var n = e === t ? null : z(e);
    (e.remove(), (e = n));
  }
}
function ur(e) {
  var t = e.parent,
    n = e.prev,
    r = e.next;
  (n !== null && (n.next = r),
    r !== null && (r.prev = n),
    t !== null && (t.first === e && (t.first = r), t.last === e && (t.last = n)));
}
function dr(e, t, n = !0) {
  var r = [];
  fr(e, r, !0);
  var i = () => {
      (n && H(e), t && t());
    },
    a = r.length;
  if (a > 0) {
    var o = () => --a || i();
    for (var s of r) s.out(o);
  } else i();
}
function fr(e, t, n) {
  if (!(e.f & 8192)) {
    e.f ^= S;
    var r = e.nodes && e.nodes.t;
    if (r !== null) for (let e of r) (e.is_global || n) && t.push(e);
    for (var i = e.first; i !== null; ) {
      var a = i.next;
      if (!(i.f & 64)) {
        var o = (i.f & 65536) != 0 || ((i.f & 32) != 0 && (e.f & 16) != 0);
        fr(i, t, o ? n : !1);
      }
      i = a;
    }
  }
}
function pr(e) {
  mr(e, !0);
}
function mr(e, t) {
  if (e.f & 8192) {
    ((e.f ^= S), e.f & 1024 || (j(e, b), Rt.ensure().schedule(e)));
    for (var n = e.first; n !== null; ) {
      var r = n.next,
        i = (n.f & 65536) != 0 || (n.f & 32) != 0;
      (mr(n, i ? t : !1), (n = r));
    }
    var a = e.nodes && e.nodes.t;
    if (a !== null) for (let e of a) (e.is_global || t) && e.in();
  }
}
function hr(e, t) {
  if (e.nodes)
    for (var n = e.nodes.start, r = e.nodes.end; n !== null; ) {
      var i = n === r ? null : z(n);
      (t.append(n), (n = i));
    }
}
var gr = null,
  _r = !1,
  vr = !1;
function yr(e) {
  vr = e;
}
var U = null,
  W = !1;
function G(e) {
  U = e;
}
var K = null;
function q(e) {
  K = e;
}
var J = null;
function br(e) {
  U !== null && (!k || U.f & 2) && (J === null ? (J = [e]) : J.push(e));
}
var Y = null,
  X = 0,
  Z = null;
function xr(e) {
  Z = e;
}
var Sr = 1,
  Cr = 0,
  wr = Cr;
function Tr(e) {
  wr = e;
}
function Er() {
  return ++Sr;
}
function Dr(e) {
  var t = e.f;
  if (t & 2048) return !0;
  if ((t & 2 && (e.f &= ~se), t & 4096)) {
    for (var n = e.deps, r = n.length, i = 0; i < r; i++) {
      var a = n[i];
      if ((Dr(a) && un(a), a.wv > e.wv)) return !0;
    }
    t & 512 && P === null && j(e, y);
  }
  return !1;
}
function Or(e, t, n = !0) {
  var r = e.reactions;
  if (r !== null && !(!k && J !== null && i.call(J, e)))
    for (var a = 0; a < r.length; a++) {
      var o = r[a];
      o.f & 2 ? Or(o, t, !1) : t === o && (n ? j(o, b) : o.f & 1024 && j(o, x), Gt(o));
    }
}
function kr(e) {
  var t = Y,
    n = X,
    r = Z,
    i = U,
    a = J,
    o = A,
    s = W,
    c = wr,
    l = e.f;
  ((Y = null),
    (X = 0),
    (Z = null),
    (U = l & 96 ? null : e),
    (J = null),
    it(e.ctx),
    (W = !1),
    (wr = ++Cr),
    e.ac !== null &&
      (Un(() => {
        e.ac.abort(he);
      }),
      (e.ac = null)));
  try {
    e.f |= ce;
    var u = e.fn,
      d = u();
    e.f |= te;
    var f = e.deps,
      p = N?.is_fork;
    if (Y !== null) {
      var m;
      if ((p || jr(e, X), f !== null && X > 0))
        for (f.length = X + Y.length, m = 0; m < Y.length; m++) f[X + m] = Y[m];
      else e.deps = f = Y;
      if (qn() && e.f & 512) for (m = X; m < f.length; m++) (f[m].reactions ??= []).push(e);
    } else !p && f !== null && X < f.length && (jr(e, X), (f.length = X));
    if (ft() && Z !== null && !W && f !== null && !(e.f & 6146))
      for (m = 0; m < Z.length; m++) Or(Z[m], e);
    if (i !== null && i !== e) {
      if ((Cr++, i.deps !== null)) for (let e = 0; e < n; e += 1) i.deps[e].rv = Cr;
      if (t !== null) for (let e of t) e.rv = Cr;
      Z !== null && (r === null ? (r = Z) : r.push(...Z));
    }
    return (e.f & 8388608 && (e.f ^= ue), d);
  } catch (e) {
    return yt(e);
  } finally {
    ((e.f ^= ce), (Y = t), (X = n), (Z = r), (U = i), (J = a), it(o), (W = s), (wr = c));
  }
}
function Ar(e, t) {
  let n = t.reactions;
  if (n !== null) {
    var a = r.call(n, e);
    if (a !== -1) {
      var o = n.length - 1;
      o === 0 ? (n = t.reactions = null) : ((n[a] = n[o]), n.pop());
    }
  }
  if (n === null && t.f & 2 && (Y === null || !i.call(Y, t))) {
    var s = t;
    (s.f & 512 && ((s.f ^= 512), (s.f &= ~se)), s.v !== C && St(s), dn(s), jr(s, 0));
  }
}
function jr(e, t) {
  var n = e.deps;
  if (n !== null) for (var r = t; r < n.length; r++) Ar(e, n[r]);
}
function Mr(e) {
  var t = e.f;
  if (!(t & 16384)) {
    j(e, y);
    var n = K,
      r = _r;
    ((K = e), (_r = !0));
    try {
      (t & 16777232 ? cr(e) : sr(e), or(e));
      var i = kr(e);
      ((e.teardown = typeof i == `function` ? i : null), (e.wv = Sr));
    } finally {
      ((_r = r), (K = n));
    }
  }
}
async function Nr() {
  if (k)
    return new Promise((e) => {
      (requestAnimationFrame(() => e()), setTimeout(() => e()));
    });
  (await Promise.resolve(), zt());
}
function Pr() {
  return Rt.ensure().settled();
}
function Q(e) {
  var t = (e.f & 2) != 0;
  if (
    (gr?.add(e), U !== null && !W && !(K !== null && K.f & 16384) && (J === null || !i.call(J, e)))
  ) {
    var n = U.deps;
    if (U.f & 2097152)
      e.rv < Cr &&
        ((e.rv = Cr),
        Y === null && n !== null && n[X] === e ? X++ : Y === null ? (Y = [e]) : Y.push(e));
    else {
      (U.deps ??= []).push(e);
      var r = e.reactions;
      r === null ? (e.reactions = [U]) : i.call(r, U) || r.push(U);
    }
  }
  if (vr && mn.has(e)) return mn.get(e);
  if (t) {
    var a = e;
    if (vr) {
      var o = a.v;
      return (((!(a.f & 1024) && a.reactions !== null) || Ir(a)) && (o = ln(a)), mn.set(a, o), o);
    }
    var s = (a.f & 512) == 0 && !W && U !== null && (_r || (U.f & 512) != 0),
      c = (a.f & te) === 0;
    (Dr(a) && (s && (a.f |= 512), un(a)), s && !c && (fn(a), Fr(a)));
  }
  if (P?.has(e)) return P.get(e);
  if (e.f & 8388608) throw e.v;
  return e.v;
}
function Fr(e) {
  if (((e.f |= 512), e.deps !== null))
    for (let t of e.deps) ((t.reactions ??= []).push(e), t.f & 2 && !(t.f & 512) && (fn(t), Fr(t)));
}
function Ir(e) {
  if (e.v === C) return !0;
  if (e.deps === null) return !1;
  for (let t of e.deps) if (mn.has(t) || (t.f & 2 && Ir(t))) return !0;
  return !1;
}
function Lr(e) {
  var t = W;
  try {
    return ((W = !0), e());
  } finally {
    W = t;
  }
}
function Rr() {
  return Symbol(Ve);
}
function zr(e) {
  return e.endsWith(`capture`) && e !== `gotpointercapture` && e !== `lostpointercapture`;
}
var Br = [
  `beforeinput`,
  `click`,
  `change`,
  `dblclick`,
  `contextmenu`,
  `focusin`,
  `focusout`,
  `input`,
  `keydown`,
  `keyup`,
  `mousedown`,
  `mousemove`,
  `mouseout`,
  `mouseover`,
  `mouseup`,
  `pointerdown`,
  `pointermove`,
  `pointerout`,
  `pointerover`,
  `pointerup`,
  `touchend`,
  `touchmove`,
  `touchstart`,
];
function Vr(e) {
  return Br.includes(e);
}
var Hr =
    `allowfullscreen.async.autofocus.autoplay.checked.controls.default.disabled.formnovalidate.indeterminate.inert.ismap.loop.multiple.muted.nomodule.novalidate.open.playsinline.readonly.required.reversed.seamless.selected.webkitdirectory.defer.disablepictureinpicture.disableremoteplayback`.split(
      `.`,
    ),
  Ur = {
    formnovalidate: `formNoValidate`,
    ismap: `isMap`,
    nomodule: `noModule`,
    playsinline: `playsInline`,
    readonly: `readOnly`,
    defaultvalue: `defaultValue`,
    defaultchecked: `defaultChecked`,
    srcobject: `srcObject`,
    novalidate: `noValidate`,
    allowfullscreen: `allowFullscreen`,
    disablepictureinpicture: `disablePictureInPicture`,
    disableremoteplayback: `disableRemotePlayback`,
  };
function Wr(e) {
  return ((e = e.toLowerCase()), Ur[e] ?? e);
}
[...Hr];
var Gr = [`touchstart`, `touchmove`];
function Kr(e) {
  return Gr.includes(e);
}
var qr = [`textarea`, `script`, `style`, `title`];
function Jr(e) {
  return qr.includes(e);
}
var Yr = Symbol(`events`),
  Xr = new Set(),
  Zr = new Set();
function Qr(e, t, n, r = {}) {
  function i(e) {
    if ((r.capture || ii.call(t, e), !e.cancelBubble)) return Un(() => n?.call(this, e));
  }
  return (
    e.startsWith(`pointer`) || e.startsWith(`touch`) || e === `wheel`
      ? _t(() => {
          t.addEventListener(e, i, r);
        })
      : t.addEventListener(e, i, r),
    i
  );
}
function $r(e, t, n, r = {}) {
  var i = Qr(t, e, n, r);
  return () => {
    e.removeEventListener(t, i, r);
  };
}
function ei(e, t, n, r, i) {
  var a = { capture: r, passive: i },
    o = Qr(e, t, n, a);
  (t === document.body || t === window || t === document || t instanceof HTMLMediaElement) &&
    Jn(() => {
      t.removeEventListener(e, o, a);
    });
}
function ti(e, t, n) {
  (t[Yr] ??= {})[e] = n;
}
function ni(e) {
  for (var t = 0; t < e.length; t++) Xr.add(e[t]);
  for (var n of Zr) n(e);
}
var ri = null;
function ii(e) {
  var t = this,
    n = t.ownerDocument,
    r = e.type,
    i = e.composedPath?.() || [],
    a = i[0] || e.target;
  ri = e;
  var s = 0,
    c = ri === e && e[Yr];
  if (c) {
    var l = i.indexOf(c);
    if (l !== -1 && (t === document || t === window)) {
      e[Yr] = t;
      return;
    }
    var u = i.indexOf(t);
    if (u === -1) return;
    l <= u && (s = l);
  }
  if (((a = i[s] || e.target), a !== t)) {
    o(e, `currentTarget`, {
      configurable: !0,
      get() {
        return a || n;
      },
    });
    var d = U,
      f = K;
    (G(null), q(null));
    try {
      for (var p, m = []; a !== null; ) {
        var h = a.assignedSlot || a.parentNode || a.host || null;
        try {
          var g = a[Yr]?.[r];
          g != null && (!a.disabled || e.target === a) && g.call(a, e);
        } catch (e) {
          p ? m.push(e) : (p = e);
        }
        if (e.cancelBubble || h === t || h === null) break;
        a = h;
      }
      if (p) {
        for (let e of m)
          queueMicrotask(() => {
            throw e;
          });
        throw p;
      }
    } finally {
      ((e[Yr] = t), delete e.currentTarget, G(d), q(f));
    }
  }
}
var ai =
  globalThis?.window?.trustedTypes &&
  globalThis.window.trustedTypes.createPolicy(`svelte-trusted-html`, { createHTML: (e) => e });
function oi(e) {
  return ai?.createHTML(e) ?? e;
}
function si(e) {
  var t = Rn(`template`);
  return ((t.innerHTML = oi(e.replaceAll(`<!>`, `<!---->`))), t.content);
}
function $(e, t) {
  var n = K;
  n.nodes === null && (n.nodes = { start: e, end: t, a: null, t: null });
}
function ci(e, t) {
  var n = (t & 1) != 0,
    r = (t & 2) != 0,
    i,
    a = !e.startsWith(`<!>`);
  return () => {
    if (w) return ($(E, null), E);
    i === void 0 && ((i = si(a ? e : `<!>` + e)), n || (i = R(i)));
    var t = r || kn ? document.importNode(i, !0) : i.cloneNode(!0);
    if (n) {
      var o = R(t),
        s = t.lastChild;
      $(o, s);
    } else $(t, t);
    return t;
  };
}
function li(e, t, n = `svg`) {
  var r = !e.startsWith(`<!>`),
    i = (t & 1) != 0,
    a = `<${n}>${r ? e : `<!>` + e}</${n}>`,
    o;
  return () => {
    if (w) return ($(E, null), E);
    if (!o) {
      var e = R(si(a));
      if (i) for (o = document.createDocumentFragment(); R(e); ) o.appendChild(R(e));
      else o = R(e);
    }
    var t = o.cloneNode(!0);
    if (i) {
      var n = R(t),
        r = t.lastChild;
      $(n, r);
    } else $(t, t);
    return t;
  };
}
function ui(e, t) {
  return li(e, t, `svg`);
}
function di(e = ``) {
  if (!w) {
    var t = L(e + ``);
    return ($(t, t), t);
  }
  var n = E;
  return (n.nodeType === 3 ? zn(n) : (n.before((n = L())), D(n)), $(n, n), n);
}
function fi() {
  if (w) return ($(E, null), E);
  var e = document.createDocumentFragment(),
    t = document.createComment(``),
    n = L();
  return (e.append(t, n), $(t, n), e);
}
function pi(e, t) {
  if (w) {
    var n = K;
    ((!(n.f & 32768) || n.nodes.end === null) && (n.nodes.end = E), O());
    return;
  }
  e !== null && e.before(t);
}
function mi() {
  if (w && E && E.nodeType === 8 && E.textContent?.startsWith(`$`)) {
    let e = E.textContent.substring(1);
    return (O(), e);
  }
  return (((window.__svelte ??= {}).uid ??= 1), `c${window.__svelte.uid++}`);
}
function hi(e, t) {
  var n = t == null ? `` : typeof t == `object` ? `${t}` : t;
  n !== (e.__t ??= e.nodeValue) && ((e.__t = n), (e.nodeValue = `${n}`));
}
function gi(e, t) {
  return yi(e, t);
}
function _i(e, t) {
  (Mn(), (t.intro = t.intro ?? !1));
  let n = t.target,
    r = w,
    i = E;
  try {
    for (var a = R(n); a && (a.nodeType !== 8 || a.data !== `[`); ) a = z(a);
    if (!a) throw Le;
    (T(!0), D(a));
    let r = yi(e, { ...t, anchor: a });
    return (T(!1), r);
  } catch (r) {
    if (
      r instanceof Error &&
      r.message
        .split(`
`)
        .some((e) => e.startsWith(`https://svelte.dev/e/`))
    )
      throw r;
    return (
      r !== Le && console.warn(`Failed to hydrate: `, r),
      t.recover === !1 && ke(),
      Mn(),
      In(n),
      T(!1),
      gi(e, t)
    );
  } finally {
    (T(r), D(i));
  }
}
var vi = new Map();
function yi(
  e,
  { target: t, anchor: n, props: r = {}, events: i, context: o, intro: s = !0, transformError: c },
) {
  Mn();
  var l = void 0,
    u = $n(() => {
      var s = n ?? t.appendChild(L());
      Zt(
        s,
        { pending: () => {} },
        (t) => {
          ut({});
          var n = A;
          if (
            (o && (n.c = o),
            i && (r.$$events = i),
            w && $(t, null),
            (l = e(t, r) || {}),
            w && ((K.nodes.end = E), E === null || E.nodeType !== 8 || E.data !== `]`))
          )
            throw (We(), Le);
          dt();
        },
        c,
      );
      var u = new Set(),
        d = (e) => {
          for (var n = 0; n < e.length; n++) {
            var r = e[n];
            if (!u.has(r)) {
              u.add(r);
              var i = Kr(r);
              for (let e of [t, document]) {
                var a = vi.get(e);
                a === void 0 && ((a = new Map()), vi.set(e, a));
                var o = a.get(r);
                o === void 0
                  ? (e.addEventListener(r, ii, { passive: i }), a.set(r, 1))
                  : a.set(r, o + 1);
              }
            }
          }
        };
      return (
        d(a(Xr)),
        Zr.add(d),
        () => {
          for (var e of u)
            for (let n of [t, document]) {
              var r = vi.get(n),
                i = r.get(e);
              --i == 0
                ? (n.removeEventListener(e, ii), r.delete(e), r.size === 0 && vi.delete(n))
                : r.set(e, i);
            }
          (Zr.delete(d), s !== n && s.parentNode?.removeChild(s));
        }
      );
    });
  return (bi.set(l, u), l);
}
var bi = new WeakMap();
function xi(e, t) {
  let n = bi.get(e);
  return n ? (bi.delete(e), n(t)) : Promise.resolve();
}
var Si = class {
  anchor;
  #e = new Map();
  #t = new Map();
  #n = new Map();
  #r = new Set();
  #i = !0;
  constructor(e, t = !0) {
    ((this.anchor = e), (this.#i = t));
  }
  #a = (e) => {
    if (this.#e.has(e)) {
      var t = this.#e.get(e),
        n = this.#t.get(t);
      if (n) (pr(n), this.#r.delete(t));
      else {
        var r = this.#n.get(t);
        r &&
          (this.#t.set(t, r.effect),
          this.#n.delete(t),
          r.fragment.lastChild.remove(),
          this.anchor.before(r.fragment),
          (n = r.effect));
      }
      for (let [t, n] of this.#e) {
        if ((this.#e.delete(t), t === e)) break;
        let r = this.#n.get(n);
        r && (H(r.effect), this.#n.delete(n));
      }
      for (let [e, r] of this.#t) {
        if (e === t || this.#r.has(e)) continue;
        let i = () => {
          if (Array.from(this.#e.values()).includes(e)) {
            var t = document.createDocumentFragment();
            (hr(r, t), t.append(L()), this.#n.set(e, { effect: r, fragment: t }));
          } else H(r);
          (this.#r.delete(e), this.#t.delete(e));
        };
        this.#i || !n ? (this.#r.add(e), dr(r, i, !1)) : i();
      }
    }
  };
  #o = (e) => {
    this.#e.delete(e);
    let t = Array.from(this.#e.values());
    for (let [e, n] of this.#n) t.includes(e) || (H(n.effect), this.#n.delete(e));
  };
  ensure(e, t) {
    var n = N,
      r = Ln();
    if (t && !this.#t.has(e) && !this.#n.has(e))
      if (r) {
        var i = document.createDocumentFragment(),
          a = L();
        (i.append(a), this.#n.set(e, { effect: V(() => t(a)), fragment: i }));
      } else
        this.#t.set(
          e,
          V(() => t(this.anchor)),
        );
    if ((this.#e.set(n, e), r)) {
      for (let [t, r] of this.#t) t === e ? n.unskip_effect(r) : n.skip_effect(r);
      for (let [t, r] of this.#n) t === e ? n.unskip_effect(r.effect) : n.skip_effect(r.effect);
      (n.oncommit(this.#a), n.ondiscard(this.#o));
    } else (w && (this.anchor = E), this.#a(n));
  }
};
function Ci(e, t, n = !1) {
  var r;
  w && ((r = E), O());
  var i = new Si(e),
    a = n ? re : 0;
  function o(e, t) {
    if (w) {
      var n = Xe(r);
      if (e !== parseInt(n.substring(1))) {
        var a = Ye();
        (D(a), (i.anchor = a), T(!1), i.ensure(e, t), T(!0));
        return;
      }
    }
    i.ensure(e, t);
  }
  ir(() => {
    var e = !1;
    (t((t, n = 0) => {
      ((e = !0), o(n, t));
    }),
      e || o(-1, null));
  }, a);
}
var wi = Symbol(`NaN`);
function Ti(e, t, n) {
  w && O();
  var r = new Si(e),
    i = !ft();
  ir(() => {
    var e = t();
    (e !== e && (e = wi), i && typeof e == `object` && e && (e = {}), r.ensure(e, n));
  });
}
function Ei(e, t) {
  return t;
}
function Di(e, t, n) {
  for (var r = [], i = t.length, o, s = t.length, c = 0; c < i; c++) {
    let n = t[c];
    dr(
      n,
      () => {
        if (o) {
          if ((o.pending.delete(n), o.done.add(n), o.pending.size === 0)) {
            var t = e.outrogroups;
            (Oi(e, a(o.done)), t.delete(o), t.size === 0 && (e.outrogroups = null));
          }
        } else --s;
      },
      !1,
    );
  }
  if (s === 0) {
    var l = r.length === 0 && n !== null;
    if (l) {
      var u = n,
        d = u.parentNode;
      (In(d), d.append(u), e.items.clear());
    }
    Oi(e, t, !l);
  } else ((o = { pending: new Set(t), done: new Set() }), (e.outrogroups ??= new Set()).add(o));
}
function Oi(e, t, n = !0) {
  var r;
  if (e.pending.size > 0) {
    r = new Set();
    for (let t of e.pending.values()) for (let n of t) r.add(e.items.get(n).e);
  }
  for (var i = 0; i < t.length; i++) {
    var a = t[i];
    r?.has(a) ? ((a.f |= oe), hr(a, document.createDocumentFragment())) : H(t[i], n);
  }
}
var ki;
function Ai(e, t, r, i, o, s = null) {
  var c = e,
    l = new Map();
  if (t & 4) {
    var u = e;
    c = w ? D(R(u)) : u.appendChild(L());
  }
  w && O();
  var d = null,
    f = sn(() => {
      var e = r();
      return n(e) ? e : e == null ? [] : a(e);
    }),
    p,
    m = new Map(),
    h = !0;
  function g(e) {
    v.effect.f & 16384 ||
      (v.pending.delete(e),
      (v.fallback = d),
      Mi(v, p, c, t, i),
      d !== null &&
        (p.length === 0
          ? d.f & 33554432
            ? ((d.f ^= oe), Pi(d, null, c))
            : pr(d)
          : dr(d, () => {
              d = null;
            })));
  }
  function _(e) {
    v.pending.delete(e);
  }
  var v = {
    effect: ir(() => {
      p = Q(f);
      var e = p.length;
      let n = !1;
      w && (Xe(c) === `[!`) != (e === 0) && ((c = Ye()), D(c), T(!1), (n = !0));
      for (var a = new Set(), u = N, v = Ln(), y = 0; y < e; y += 1) {
        w && E.nodeType === 8 && E.data === `]` && ((c = E), (n = !0), T(!1));
        var b = p[y],
          x = i(b, y),
          S = h ? null : l.get(x);
        (S
          ? (S.v && bn(S.v, b), S.i && bn(S.i, y), v && u.unskip_effect(S.e))
          : ((S = Ni(l, h ? c : (ki ??= L()), b, x, y, o, t, r)), h || (S.e.f |= oe), l.set(x, S)),
          a.add(x));
      }
      if (
        (e === 0 &&
          s &&
          !d &&
          (h ? (d = V(() => s(c))) : ((d = V(() => s((ki ??= L())))), (d.f |= oe))),
        e > a.size && xe(``, ``, ``),
        w && e > 0 && D(Ye()),
        !h)
      )
        if ((m.set(u, a), v)) {
          for (let [e, t] of l) a.has(e) || u.skip_effect(t.e);
          (u.oncommit(g), u.ondiscard(_));
        } else g(u);
      (n && T(!0), Q(f));
    }),
    flags: t,
    items: l,
    pending: m,
    outrogroups: null,
    fallback: d,
  };
  ((h = !1), w && (c = E));
}
function ji(e) {
  for (; e !== null && !(e.f & 32); ) e = e.next;
  return e;
}
function Mi(e, t, n, r, i) {
  var o = (r & 8) != 0,
    s = t.length,
    c = e.items,
    l = ji(e.effect.first),
    u,
    d = null,
    f,
    p = [],
    m = [],
    h,
    g,
    _,
    v;
  if (o)
    for (v = 0; v < s; v += 1)
      ((h = t[v]),
        (g = i(h, v)),
        (_ = c.get(g).e),
        _.f & 33554432 || (_.nodes?.a?.measure(), (f ??= new Set()).add(_)));
  for (v = 0; v < s; v += 1) {
    if (((h = t[v]), (g = i(h, v)), (_ = c.get(g).e), e.outrogroups !== null))
      for (let t of e.outrogroups) (t.pending.delete(_), t.done.delete(_));
    if (
      (_.f & 8192 && (pr(_), o && (_.nodes?.a?.unfix(), (f ??= new Set()).delete(_))),
      _.f & 33554432)
    )
      if (((_.f ^= oe), _ === l)) Pi(_, null, n);
      else {
        var y = d ? d.next : l;
        (_ === e.effect.last && (e.effect.last = _.prev),
          _.prev && (_.prev.next = _.next),
          _.next && (_.next.prev = _.prev),
          Fi(e, d, _),
          Fi(e, _, y),
          Pi(_, y, n),
          (d = _),
          (p = []),
          (m = []),
          (l = ji(d.next)));
        continue;
      }
    if (_ !== l) {
      if (u !== void 0 && u.has(_)) {
        if (p.length < m.length) {
          var b = m[0],
            x;
          d = b.prev;
          var S = p[0],
            ee = p[p.length - 1];
          for (x = 0; x < p.length; x += 1) Pi(p[x], b, n);
          for (x = 0; x < m.length; x += 1) u.delete(m[x]);
          (Fi(e, S.prev, ee.next),
            Fi(e, d, S),
            Fi(e, ee, b),
            (l = b),
            (d = ee),
            --v,
            (p = []),
            (m = []));
        } else
          (u.delete(_),
            Pi(_, l, n),
            Fi(e, _.prev, _.next),
            Fi(e, _, d === null ? e.effect.first : d.next),
            Fi(e, d, _),
            (d = _));
        continue;
      }
      for (p = [], m = []; l !== null && l !== _; )
        ((u ??= new Set()).add(l), m.push(l), (l = ji(l.next)));
      if (l === null) continue;
    }
    (_.f & 33554432 || p.push(_), (d = _), (l = ji(_.next)));
  }
  if (e.outrogroups !== null) {
    for (let t of e.outrogroups)
      t.pending.size === 0 && (Oi(e, a(t.done)), e.outrogroups?.delete(t));
    e.outrogroups.size === 0 && (e.outrogroups = null);
  }
  if (l !== null || u !== void 0) {
    var te = [];
    if (u !== void 0) for (_ of u) _.f & 8192 || te.push(_);
    for (; l !== null; ) (!(l.f & 8192) && l !== e.fallback && te.push(l), (l = ji(l.next)));
    var ne = te.length;
    if (ne > 0) {
      var re = r & 4 && s === 0 ? n : null;
      if (o) {
        for (v = 0; v < ne; v += 1) te[v].nodes?.a?.measure();
        for (v = 0; v < ne; v += 1) te[v].nodes?.a?.fix();
      }
      Di(e, te, re);
    }
  }
  o &&
    _t(() => {
      if (f !== void 0) for (_ of f) _.nodes?.a?.apply();
    });
}
function Ni(e, t, n, r, i, a, o, s) {
  var c = o & 1 ? (o & 16 ? _n(n) : yn(n, !1, !1)) : null,
    l = o & 2 ? _n(i) : null;
  return {
    v: c,
    i: l,
    e: V(
      () => (
        a(t, c ?? n, l ?? i, s),
        () => {
          e.delete(r);
        }
      ),
    ),
  };
}
function Pi(e, t, n) {
  if (e.nodes)
    for (
      var r = e.nodes.start, i = e.nodes.end, a = t && !(t.f & 33554432) ? t.nodes.start : n;
      r !== null;
    ) {
      var o = z(r);
      if ((a.before(r), r === i)) return;
      r = o;
    }
}
function Fi(e, t, n) {
  (t === null ? (e.effect.first = n) : (t.next = n),
    n === null ? (e.effect.last = t) : (n.prev = t));
}
function Ii(e, t, n = !1, r = !1, i = !1, a = !1) {
  var o = e,
    s = ``;
  if (n) {
    var c = e;
    w && (o = D(R(c)));
  }
  rr(() => {
    var e = K;
    if (s === (s = t() ?? ``)) {
      w && O();
      return;
    }
    if (n && !w) {
      ((e.nodes = null), (c.innerHTML = s), s !== `` && $(R(c), c.lastChild));
      return;
    }
    if ((e.nodes !== null && (lr(e.nodes.start, e.nodes.end), (e.nodes = null)), s !== ``)) {
      if (w) {
        for (var a = E.data, l = O(), u = l; l !== null && (l.nodeType !== 8 || l.data !== ``); )
          ((u = l), (l = z(l)));
        if (l === null) throw (We(), Le);
        ($(E, u), (o = D(l)));
        return;
      }
      var d = Rn(r ? `svg` : i ? `math` : `template`, r ? ze : i ? Be : void 0);
      d.innerHTML = s;
      var f = r || i ? d : d.content;
      if (($(R(f), f.lastChild), r || i)) for (; R(f); ) o.before(R(f));
      else o.before(f);
    }
  });
}
function Li(e, t, ...n) {
  var r = new Si(e);
  ir(() => {
    let e = t() ?? null;
    r.ensure(e, e && ((t) => e(t, ...n)));
  }, re);
}
function Ri(e) {
  return (t, ...n) => {
    var r = e(...n),
      i;
    w ? ((i = E), O()) : ((i = R(si(r.render().trim()))), t.before(i));
    let a = r.setup?.(i);
    ($(i, i), typeof a == `function` && Jn(a));
  };
}
function zi(e, t, n) {
  var r;
  w && ((r = E), O());
  var i = new Si(e);
  ir(() => {
    var e = t() ?? null;
    if (w && (Xe(r) === `[`) != (e !== null)) {
      var a = Ye();
      (D(a), (i.anchor = a), T(!1), i.ensure(e, e && ((t) => n(t, e))), T(!0));
      return;
    }
    i.ensure(e, e && ((t) => n(t, e)));
  }, re);
}
function Bi(e, t, n, r, i, a) {
  let o = w;
  w && O();
  var s = null;
  w && E.nodeType === 1 && ((s = E), O());
  var c = w ? E : e,
    l = new Si(c, !1);
  (ir(() => {
    let e = t() || null;
    var a = i ? i() : n || e === `svg` ? ze : void 0;
    if (e === null) {
      l.ensure(null, null);
      return;
    }
    return (
      l.ensure(e, (t) => {
        if (e) {
          if (((s = w ? s : Rn(e, a)), $(s, s), r)) {
            w && Jr(e) && s.append(document.createComment(``));
            var n = w ? R(s) : s.appendChild(L());
            (w && (n === null ? T(!1) : D(n)), r(s, n));
          }
          ((K.nodes.end = s), t.before(s));
        }
        w && D(t);
      }),
      () => {}
    );
  }, re),
    Jn(() => {}),
    o && (T(!0), D(c)));
}
function Vi(e, t) {
  var n = void 0,
    r;
  ar(() => {
    n !== (n = t()) &&
      ((r &&= (H(r), null)),
      n &&
        (r = V(() => {
          er(() => n(e));
        })));
  });
}
function Hi(e) {
  var t,
    n,
    r = ``;
  if (typeof e == `string` || typeof e == `number`) r += e;
  else if (typeof e == `object`)
    if (Array.isArray(e)) {
      var i = e.length;
      for (t = 0; t < i; t++) e[t] && (n = Hi(e[t])) && (r && (r += ` `), (r += n));
    } else for (n in e) e[n] && (r && (r += ` `), (r += n));
  return r;
}
function Ui() {
  for (var e, t, n = 0, r = ``, i = arguments.length; n < i; n++)
    (e = arguments[n]) && (t = Hi(e)) && (r && (r += ` `), (r += t));
  return r;
}
function Wi(e) {
  return typeof e == `object` ? Ui(e) : (e ?? ``);
}
var Gi = [
  ...` 	
\r\f\xA0\v﻿`,
];
function Ki(e, t, n) {
  var r = e == null ? `` : `` + e;
  if ((t && (r = r ? r + ` ` + t : t), n)) {
    for (var i of Object.keys(n))
      if (n[i]) r = r ? r + ` ` + i : i;
      else if (r.length)
        for (var a = i.length, o = 0; (o = r.indexOf(i, o)) >= 0; ) {
          var s = o + a;
          (o === 0 || Gi.includes(r[o - 1])) && (s === r.length || Gi.includes(r[s]))
            ? (r = (o === 0 ? `` : r.substring(0, o)) + r.substring(s + 1))
            : (o = s);
        }
  }
  return r === `` ? null : r;
}
function qi(e, t = !1) {
  var n = t ? ` !important;` : `;`,
    r = ``;
  for (var i of Object.keys(e)) {
    var a = e[i];
    a != null && a !== `` && (r += ` ` + i + `: ` + a + n);
  }
  return r;
}
function Ji(e) {
  return e[0] !== `-` || e[1] !== `-` ? e.toLowerCase() : e;
}
function Yi(e, t) {
  if (t) {
    var n = ``,
      r,
      i;
    if ((Array.isArray(t) ? ((r = t[0]), (i = t[1])) : (r = t), e)) {
      e = String(e)
        .replaceAll(/\s*\/\*.*?\*\/\s*/g, ``)
        .trim();
      var a = !1,
        o = 0,
        s = !1,
        c = [];
      (r && c.push(...Object.keys(r).map(Ji)), i && c.push(...Object.keys(i).map(Ji)));
      var l = 0,
        u = -1;
      let t = e.length;
      for (var d = 0; d < t; d++) {
        var f = e[d];
        if (
          (s
            ? f === `/` && e[d - 1] === `*` && (s = !1)
            : a
              ? a === f && (a = !1)
              : f === `/` && e[d + 1] === `*`
                ? (s = !0)
                : f === `"` || f === `'`
                  ? (a = f)
                  : f === `(`
                    ? o++
                    : f === `)` && o--,
          !s && a === !1 && o === 0)
        ) {
          if (f === `:` && u === -1) u = d;
          else if (f === `;` || d === t - 1) {
            if (u !== -1) {
              var p = Ji(e.substring(l, u).trim());
              if (!c.includes(p)) {
                f !== `;` && d++;
                var m = e.substring(l, d).trim();
                n += ` ` + m + `;`;
              }
            }
            ((l = d + 1), (u = -1));
          }
        }
      }
    }
    return (r && (n += qi(r)), i && (n += qi(i, !0)), (n = n.trim()), n === `` ? null : n);
  }
  return e == null ? null : String(e);
}
function Xi(e, t, n, r, i, a) {
  var o = e.__className;
  if (w || o !== n || o === void 0) {
    var s = Ki(n, r, a);
    ((!w || s !== e.getAttribute(`class`)) &&
      (s == null ? e.removeAttribute(`class`) : t ? (e.className = s) : e.setAttribute(`class`, s)),
      (e.__className = n));
  } else if (a && i !== a)
    for (var c in a) {
      var l = !!a[c];
      (i == null || l !== !!i[c]) && e.classList.toggle(c, l);
    }
  return a;
}
function Zi(e, t = {}, n, r) {
  for (var i in n) {
    var a = n[i];
    t[i] !== a && (n[i] == null ? e.style.removeProperty(i) : e.style.setProperty(i, a, r));
  }
}
function Qi(e, t, n, r) {
  var i = e.__style;
  if (w || i !== t) {
    var a = Yi(t, r);
    ((!w || a !== e.getAttribute(`style`)) &&
      (a == null ? e.removeAttribute(`style`) : (e.style.cssText = a)),
      (e.__style = t));
  } else
    r && (Array.isArray(r) ? (Zi(e, n?.[0], r[0]), Zi(e, n?.[1], r[1], `important`)) : Zi(e, n, r));
  return r;
}
function $i(e, t, r = !1) {
  if (e.multiple) {
    if (t == null) return;
    if (!n(t)) return Ge();
    for (var i of e.options) i.selected = t.includes(ta(i));
    return;
  }
  for (i of e.options)
    if (Dn(ta(i), t)) {
      i.selected = !0;
      return;
    }
  (!r || t !== void 0) && (e.selectedIndex = -1);
}
function ea(e) {
  var t = new MutationObserver(() => {
    $i(e, e.__value);
  });
  (t.observe(e, { childList: !0, subtree: !0, attributes: !0, attributeFilter: [`value`] }),
    Jn(() => {
      t.disconnect();
    }));
}
function ta(e) {
  return `__value` in e ? e.__value : e.value;
}
var na = Symbol(`class`),
  ra = Symbol(`style`),
  ia = Symbol(`is custom element`),
  aa = Symbol(`is html`),
  oa = ge ? `link` : `LINK`,
  sa = ge ? `input` : `INPUT`,
  ca = ge ? `option` : `OPTION`,
  la = ge ? `select` : `SELECT`,
  ua = ge ? `progress` : `PROGRESS`;
function da(e) {
  if (w) {
    var t = !1,
      n = () => {
        if (!t) {
          if (((t = !0), e.hasAttribute(`value`))) {
            var n = e.value;
            (ha(e, `value`, null), (e.value = n));
          }
          if (e.hasAttribute(`checked`)) {
            var r = e.checked;
            (ha(e, `checked`, null), (e.checked = r));
          }
        }
      };
    ((e.__on_r = n), _t(n), Hn());
  }
}
function fa(e, t) {
  var n = va(e);
  n.value === (n.value = t ?? void 0) ||
    (e.value === t && (t !== 0 || e.nodeName !== ua)) ||
    (e.value = t ?? ``);
}
function pa(e, t) {
  var n = va(e);
  n.checked !== (n.checked = t ?? void 0) && (e.checked = t);
}
function ma(e, t) {
  t ? e.hasAttribute(`selected`) || e.setAttribute(`selected`, ``) : e.removeAttribute(`selected`);
}
function ha(e, t, n, r) {
  var i = va(e);
  (w &&
    ((i[t] = e.getAttribute(t)),
    t === `src` || t === `srcset` || (t === `href` && e.nodeName === oa))) ||
    (i[t] !== (i[t] = n) &&
      (t === `loading` && (e[pe] = n),
      n == null
        ? e.removeAttribute(t)
        : typeof n != `string` && ba(e).includes(t)
          ? (e[t] = n)
          : e.setAttribute(t, n)));
}
function ga(e, t, n, r, i = !1, a = !1) {
  if (w && i && e.nodeName === sa) {
    var o = e;
    (o.type === `checkbox` ? `defaultChecked` : `defaultValue`) in n || da(o);
  }
  var s = va(e),
    c = s[ia],
    l = !s[aa];
  let u = w && c;
  u && T(!1);
  var d = t || {},
    f = e.nodeName === ca;
  for (var p in t) p in n || (n[p] = null);
  (n.class ? (n.class = Wi(n.class)) : (r || n[na]) && (n.class = null),
    n[ra] && (n.style ??= null));
  var m = ba(e);
  for (let i in n) {
    let o = n[i];
    if (f && i === `value` && o == null) {
      ((e.value = e.__value = ``), (d[i] = o));
      continue;
    }
    if (i === `class`) {
      (Xi(e, e.namespaceURI === `http://www.w3.org/1999/xhtml`, o, r, t?.[na], n[na]),
        (d[i] = o),
        (d[na] = n[na]));
      continue;
    }
    if (i === `style`) {
      (Qi(e, o, t?.[ra], n[ra]), (d[i] = o), (d[ra] = n[ra]));
      continue;
    }
    var h = d[i];
    if (!(o === h && !(o === void 0 && e.hasAttribute(i)))) {
      d[i] = o;
      var g = i[0] + i[1];
      if (g !== `$$`)
        if (g === `on`) {
          let t = {},
            n = `$$` + i,
            r = i.slice(2);
          var _ = Vr(r);
          if ((zr(r) && ((r = r.slice(0, -7)), (t.capture = !0)), !_ && h)) {
            if (o != null) continue;
            (e.removeEventListener(r, d[n], t), (d[n] = null));
          }
          if (_) (ti(r, e, o), ni([r]));
          else if (o != null) {
            function a(e) {
              d[i].call(this, e);
            }
            d[n] = Qr(r, e, a, t);
          }
        } else if (i === `style`) ha(e, i, o);
        else if (i === `autofocus`) Bn(e, !!o);
        else if (!c && (i === `__value` || (i === `value` && o != null))) e.value = e.__value = o;
        else if (i === `selected` && f) ma(e, o);
        else {
          var v = i;
          l || (v = Wr(v));
          var y = v === `defaultValue` || v === `defaultChecked`;
          if (o == null && !c && !y)
            if (((s[i] = null), v === `value` || v === `checked`)) {
              let n = e,
                r = t === void 0;
              if (v === `value`) {
                let e = n.defaultValue;
                (n.removeAttribute(v), (n.defaultValue = e), (n.value = n.__value = r ? e : null));
              } else {
                let e = n.defaultChecked;
                (n.removeAttribute(v), (n.defaultChecked = e), (n.checked = r ? e : !1));
              }
            } else e.removeAttribute(i);
          else
            y || (m.includes(v) && (c || typeof o != `string`))
              ? ((e[v] = o), v in s && (s[v] = C))
              : typeof o != `function` && ha(e, v, o, a);
        }
    }
  }
  return (u && T(!0), d);
}
function _a(e, t, n = [], r = [], i = [], a, o = !1, s = !1) {
  $t(i, n, r, (n) => {
    var r = void 0,
      i = {},
      c = e.nodeName === la,
      l = !1;
    if (
      (ar(() => {
        var u = t(...n.map(Q)),
          d = ga(e, r, u, a, o, s);
        l && c && `value` in u && $i(e, u.value);
        for (let e of Object.getOwnPropertySymbols(i)) u[e] || H(i[e]);
        for (let t of Object.getOwnPropertySymbols(u)) {
          var f = u[t];
          (t.description === `@attach` &&
            (!r || f !== r[t]) &&
            (i[t] && H(i[t]), (i[t] = V(() => Vi(e, () => f)))),
            (d[t] = f));
        }
        r = d;
      }),
      c)
    ) {
      var u = e;
      er(() => {
        ($i(u, r.value, !0), ea(u));
      });
    }
    l = !0;
  });
}
function va(e) {
  return (e.__attributes ??= { [ia]: e.nodeName.includes(`-`), [aa]: e.namespaceURI === Re });
}
var ya = new Map();
function ba(e) {
  var t = e.getAttribute(`is`) || e.nodeName,
    n = ya.get(t);
  if (n) return n;
  ya.set(t, (n = []));
  for (var r, i = e, a = Element.prototype; a !== i; ) {
    for (var o in ((r = c(i)), r)) r[o].set && n.push(o);
    i = d(i);
  }
  return n;
}
function xa(e, t, n = t) {
  var r = new WeakSet();
  (Wn(e, `input`, async (i) => {
    var a = i ? e.defaultValue : e.value;
    if (((a = Sa(e) ? Ca(a) : a), n(a), N !== null && r.add(N), await Nr(), a !== (a = t()))) {
      var o = e.selectionStart,
        s = e.selectionEnd,
        c = e.value.length;
      if (((e.value = a ?? ``), s !== null)) {
        var l = e.value.length;
        o === s && s === c && l > c
          ? ((e.selectionStart = l), (e.selectionEnd = l))
          : ((e.selectionStart = o), (e.selectionEnd = Math.min(s, l)));
      }
    }
  }),
    ((w && e.defaultValue !== e.value) || (Lr(t) == null && e.value)) &&
      (n(Sa(e) ? Ca(e.value) : e.value), N !== null && r.add(N)),
    nr(() => {
      var n = t();
      if (e === document.activeElement) {
        var i = k ? At : N;
        if (r.has(i)) return;
      }
      (Sa(e) && n === Ca(e.value)) ||
        (e.type === `date` && !n && !e.value) ||
        (n !== e.value && (e.value = n ?? ``));
    }));
}
function Sa(e) {
  var t = e.type;
  return t === `number` || t === `range`;
}
function Ca(e) {
  return e === `` ? null : +e;
}
function wa(e, t) {
  return e === t || e?.[de] === t;
}
function Ta(e = {}, t, n, r) {
  var i = A.r,
    a = K;
  return (
    er(() => {
      var o, s;
      return (
        nr(() => {
          ((o = s),
            (s = r?.() || []),
            Lr(() => {
              e !== n(...s) && (t(e, ...s), o && wa(n(...o), e) && t(null, ...o));
            }));
        }),
        () => {
          let r = a;
          for (; r !== i && r.parent !== null && r.parent.f & 33554432; ) r = r.parent;
          let o = () => {
              s && wa(n(...s), e) && t(null, ...s);
            },
            c = r.teardown;
          r.teardown = () => {
            (o(), c?.());
          };
        }
      );
    }),
    e
  );
}
var Ea = {
  get(e, t) {
    if (!e.exclude.includes(t)) return e.props[t];
  },
  set(e, t) {
    return !1;
  },
  getOwnPropertyDescriptor(e, t) {
    if (!e.exclude.includes(t) && t in e.props)
      return { enumerable: !0, configurable: !0, value: e.props[t] };
  },
  has(e, t) {
    return e.exclude.includes(t) ? !1 : t in e.props;
  },
  ownKeys(e) {
    return Reflect.ownKeys(e.props).filter((t) => !e.exclude.includes(t));
  },
};
function Da(e, t, n) {
  return new Proxy({ props: e, exclude: t }, Ea);
}
var Oa = {
  get(e, t) {
    let n = e.props.length;
    for (; n--; ) {
      let r = e.props[n];
      if ((p(r) && (r = r()), typeof r == `object` && r && t in r)) return r[t];
    }
  },
  set(e, t, n) {
    let r = e.props.length;
    for (; r--; ) {
      let i = e.props[r];
      p(i) && (i = i());
      let a = s(i, t);
      if (a && a.set) return (a.set(n), !0);
    }
    return !1;
  },
  getOwnPropertyDescriptor(e, t) {
    let n = e.props.length;
    for (; n--; ) {
      let r = e.props[n];
      if ((p(r) && (r = r()), typeof r == `object` && r && t in r)) {
        let e = s(r, t);
        return (e && !e.configurable && (e.configurable = !0), e);
      }
    }
  },
  has(e, t) {
    if (t === de || t === fe) return !1;
    for (let n of e.props) if ((p(n) && (n = n()), n != null && t in n)) return !0;
    return !1;
  },
  ownKeys(e) {
    let t = [];
    for (let n of e.props)
      if ((p(n) && (n = n()), n)) {
        for (let e in n) t.includes(e) || t.push(e);
        for (let e of Object.getOwnPropertySymbols(n)) t.includes(e) || t.push(e);
      }
    return t;
  },
};
function ka(...e) {
  return new Proxy({ props: e }, Oa);
}
function Aa(e, t, n, r) {
  var i = !et || (n & 2) != 0,
    a = (n & 8) != 0,
    o = (n & 16) != 0,
    c = r,
    l = !0,
    u = () => (l && ((l = !1), (c = o ? Lr(r) : r)), c);
  let d;
  if (a) {
    var f = de in e || fe in e;
    d = s(e, t)?.set ?? (f && t in e ? (n) => (e[t] = n) : void 0);
  }
  var p,
    m = !1;
  (a ? ([p, m] = kt(() => e[t])) : (p = e[t]),
    p === void 0 && r !== void 0 && ((p = u()), d && (i && je(t), d(p))));
  var h = i
    ? () => {
        var n = e[t];
        return n === void 0 ? u() : ((l = !0), n);
      }
    : () => {
        var n = e[t];
        return (n !== void 0 && (c = void 0), n === void 0 ? c : n);
      };
  if (i && !(n & 4)) return h;
  if (d) {
    var g = e.$$legacy;
    return function (e, t) {
      return arguments.length > 0 ? ((!i || !t || g || m) && d(t ? h() : e), e) : h();
    };
  }
  var _ = !1,
    v = (n & 1 ? rn : sn)(() => ((_ = !1), h()));
  a && Q(v);
  var y = K;
  return function (e, t) {
    if (arguments.length > 0) {
      let n = t ? Q(v) : i && a ? Tn(e) : e;
      return (I(v, n), (_ = !0), c !== void 0 && (c = n), e);
    }
    return (vr && _) || y.f & 16384 ? v.v : Q(v);
  };
}
function ja(e) {
  return class extends Ma {
    constructor(t) {
      super({ component: e, ...t });
    }
  };
}
var Ma = class {
  #e;
  #t;
  constructor(e) {
    var t = new Map(),
      n = (e, n) => {
        var r = yn(n, !1, !1);
        return (t.set(e, r), r);
      };
    let r = new Proxy(
      { ...(e.props || {}), $$events: {} },
      {
        get(e, r) {
          return Q(t.get(r) ?? n(r, Reflect.get(e, r)));
        },
        has(e, r) {
          return r === fe ? !0 : (Q(t.get(r) ?? n(r, Reflect.get(e, r))), Reflect.has(e, r));
        },
        set(e, r, i) {
          return (I(t.get(r) ?? n(r, i), i), Reflect.set(e, r, i));
        },
      },
    );
    ((this.#t = (e.hydrate ? _i : gi)(e.component, {
      target: e.target,
      anchor: e.anchor,
      props: r,
      context: e.context,
      intro: e.intro ?? !1,
      recover: e.recover,
      transformError: e.transformError,
    })),
      !k && (!e?.props?.$$host || e.sync === !1) && zt(),
      (this.#e = r.$$events));
    for (let e of Object.keys(this.#t))
      e === `$set` ||
        e === `$destroy` ||
        e === `$on` ||
        o(this, e, {
          get() {
            return this.#t[e];
          },
          set(t) {
            this.#t[e] = t;
          },
          enumerable: !0,
        });
    ((this.#t.$set = (e) => {
      Object.assign(r, e);
    }),
      (this.#t.$destroy = () => {
        xi(this.#t);
      }));
  }
  $set(e) {
    this.#t.$set(e);
  }
  $on(e, t) {
    this.#e[e] = this.#e[e] || [];
    let n = (...e) => t.call(this, ...e);
    return (
      this.#e[e].push(n),
      () => {
        this.#e[e] = this.#e[e].filter((e) => e !== n);
      }
    );
  }
  $destroy() {
    this.#t.$destroy();
  }
};
function Na(e, t) {
  if ((k || _e(`hydratable`), w)) {
    let t = window.__svelte?.h;
    if (t?.has(e)) return t.get(e);
    Ue(e);
  }
  return t();
}
var Pa = t({
  afterUpdate: () => Va,
  beforeUpdate: () => Ba,
  createContext: () => at,
  createEventDispatcher: () => za,
  createRawSnippet: () => Ri,
  flushSync: () => zt,
  fork: () => Jt,
  getAbortSignal: () => Fa,
  getAllContexts: () => lt,
  getContext: () => ot,
  hasContext: () => ct,
  hydratable: () => Na,
  hydrate: () => _i,
  mount: () => gi,
  onDestroy: () => La,
  onMount: () => Ia,
  setContext: () => st,
  settled: () => Pr,
  tick: () => Nr,
  unmount: () => xi,
  untrack: () => Lr,
});
function Fa() {
  return (U === null && Oe(), (U.ac ??= new AbortController()).signal);
}
function Ia(e) {
  (A === null && ve(`onMount`),
    et && A.l !== null
      ? Ha(A).m.push(e)
      : Yn(() => {
          let t = Lr(e);
          if (typeof t == `function`) return t;
        }));
}
function La(e) {
  (A === null && ve(`onDestroy`), Ia(() => () => Lr(e)));
}
function Ra(e, t, { bubbles: n = !1, cancelable: r = !1 } = {}) {
  return new CustomEvent(e, { detail: t, bubbles: n, cancelable: r });
}
function za() {
  let e = A;
  return (
    e === null && ve(`createEventDispatcher`),
    (t, r, i) => {
      let a = e.s.$$events?.[t];
      if (a) {
        let o = n(a) ? a.slice() : [a],
          s = Ra(t, r, i);
        for (let t of o) t.call(e.x, s);
        return !s.defaultPrevented;
      }
      return !0;
    }
  );
}
function Ba(e) {
  (A === null && ve(`beforeUpdate`), A.l === null && Ae(`beforeUpdate`), Ha(A).b.push(e));
}
function Va(e) {
  (A === null && ve(`afterUpdate`), A.l === null && Ae(`afterUpdate`), Ha(A).a.push(e));
}
function Ha(e) {
  var t = e.l;
  return (t.u ??= { a: [], b: [], m: [] });
}
var Ua = `modulepreload`,
  Wa = function (e, t) {
    return new URL(e, t).href;
  },
  Ga = {},
  Ka = function (e, t, n) {
    let r = Promise.resolve();
    if (t && t.length > 0) {
      let e = document.getElementsByTagName(`link`),
        i = document.querySelector(`meta[property=csp-nonce]`),
        a = i?.nonce || i?.getAttribute(`nonce`);
      function o(e) {
        return Promise.all(
          e.map((e) =>
            Promise.resolve(e).then(
              (e) => ({ status: `fulfilled`, value: e }),
              (e) => ({ status: `rejected`, reason: e }),
            ),
          ),
        );
      }
      r = o(
        t.map((t) => {
          if (((t = Wa(t, n)), t in Ga)) return;
          Ga[t] = !0;
          let r = t.endsWith(`.css`),
            i = r ? `[rel="stylesheet"]` : ``;
          if (n)
            for (let n = e.length - 1; n >= 0; n--) {
              let i = e[n];
              if (i.href === t && (!r || i.rel === `stylesheet`)) return;
            }
          else if (document.querySelector(`link[href="${t}"]${i}`)) return;
          let o = document.createElement(`link`);
          if (
            ((o.rel = r ? `stylesheet` : Ua),
            r || (o.as = `script`),
            (o.crossOrigin = ``),
            (o.href = t),
            a && o.setAttribute(`nonce`, a),
            document.head.appendChild(o),
            r)
          )
            return new Promise((e, n) => {
              (o.addEventListener(`load`, e),
                o.addEventListener(`error`, () => n(Error(`Unable to preload CSS for ${t}`))));
            });
        }),
      );
    }
    function i(e) {
      let t = new Event(`vite:preloadError`, { cancelable: !0 });
      if (((t.payload = e), window.dispatchEvent(t), !t.defaultPrevented)) throw e;
    }
    return r.then((t) => {
      for (let e of t || []) e.status === `rejected` && i(e.reason);
      return e().catch(i);
    });
  };
export {
  Zn as $,
  Ci as A,
  ni as B,
  Bi as C,
  m as Ct,
  Ai as D,
  Ii as E,
  fi as F,
  Q as G,
  ei as H,
  ci as I,
  Lr as J,
  Pr as K,
  ui as L,
  hi as M,
  xi as N,
  Ei as O,
  pi as P,
  Yn as Q,
  mi as R,
  Vi as S,
  qe as St,
  Li as T,
  t as Tt,
  $r as U,
  ti as V,
  Rr as W,
  Qn as X,
  wr as Y,
  rr as Z,
  $i as _,
  ut as _t,
  ja as a,
  Cn as at,
  Wi as b,
  tt as bt,
  ka as c,
  vn as ct,
  _a as d,
  Yt as dt,
  On as et,
  da as f,
  Et as ft,
  ea as g,
  dt as gt,
  fa as h,
  ct as ht,
  Ia as i,
  Tn as it,
  gi as j,
  Ti as k,
  Ta as l,
  Sn as lt,
  pa as m,
  ot as mt,
  Pa as n,
  Pn as nt,
  Aa as o,
  I as ot,
  ha as p,
  lt as pt,
  Nr as q,
  La as r,
  Fn as rt,
  Da as s,
  _n as st,
  Ka as t,
  Nn as tt,
  xa as u,
  on as ut,
  Qi as v,
  st as vt,
  zi as w,
  _ as wt,
  Ui as x,
  Je as xt,
  Xi as y,
  rt as yt,
  di as z,
};
